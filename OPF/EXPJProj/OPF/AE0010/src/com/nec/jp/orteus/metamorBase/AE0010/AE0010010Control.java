...
//20150805 ADD END BY SYSCOM
         ...
         //Add Start 20140108 zhang-zl
         // [発注残].”増額発注フラグ”により、画面の増額発注チェックボックスをセット
		...
		logger.entering("AE0010010Control"+":USER="+struct.getsUser_ID(),"controlInsert");
			//{{user_implement_dev:<controlInsert>
                try {
                 ...
                 // 備品発注でかつ、｢品目番号｣がnullの場合以下のチェックを行わない
                 AE0010010Struct itemStruct = null;
                 if (!("true".equals(struct.getc_NON_NO_ITEM_FLG())
                 ...
                 // 「増額発注」 ＝ チェックOn、「元発注番号」 で [発注残] を検索し、存在しなかった場合
...
//20150805 ADD START BY SYSCOM
                 // ｢負担部門コード｣ != Nullで、かつ[負担部門コードView]に存在しないとき
...
//20150805 ADD END BY SYSCOM
                ...
                //内税の場合に税額を計算する
                 ...
                 //Add Start 20140113 zhang-zl
                 // 画面の増額発注チェックボックスにより、[発注残].”増額発注フラグ”をセット
                 if ("true".equals(struct.getc_ADD_AMOUNT_PUCH_ODR_FLG())) {
                ...
                try {
                 ...
                 if (!"true".equals(struct.getc_NON_NO_ITEM_FLG())) {
                  ...
                  if (odStruct.getJOB_ODR_CD() != null) {
                   // ｢製番｣で、[製番計画]を検索し、[製番計画]."納期" < ｢発注納期｣の場合
...
//	 	                          struct.getPUCH_ODR_QTY());
                 ...
                 // 備品発注でかつ、品目番号がNullの場合は以下の処理を行わない
                 if (!("true".equals(struct.getc_NON_NO_ITEM_FLG())
                  && (struct.getITEM_CD() == null || struct.getITEM_CD().length() <= 0))) {
                  // 整数管理品目の場合、発注数を切り上げ
                  if ("1".equals(itemStruct.getUNIT_QTY_TYP())) {
            	...
            	//20090427 MOD END
                  ...
                  // 備品購入フラグが、チェックオフ（通常品）の場合、[所要量]の更新を行う。
                  if (!"1".equals(struct.getNON_NO_ITEM_FLG())) {
...
//			e.printStackTrace();
//			throw new FoundationException("AE0010010Control",e.getMessage(),"control_eventHandllerメソッドでエラーが発生しました。");
	...
	public void createDefaultCSV(List list, String sTitle[]) throws BusinessProcessException, FoundationException {
		...
		//{{user_implement_dev:<createDefaultCSV>
			...
			e.printStackTrace();
//			objMessage.m_writer.write(Level.ALL, objMessage.getIDMsg("AE0010010-E999","CSVの出力処理"));
			throw new FoundationException("AE0010010Control",e.getMessage(),"createDefaultCSVメソッドでエラーが発生しました。");
		}
...
//				objMessage.m_writer.write(Level.ALL, objMessage.getIDMsg("AE0010010-E999","システム日付の取得処理"));
				throw new FoundationException("AE0010010Control","beginTransaction()","システム日付取得処理でエラーが発生しました。");
			}
		...
		//{{user_implement_dev:<beginTransaction>
			...
			e.printStackTrace();
//			objMessage.m_writer.write(Level.ALL, objMessage.getIDMsg("AE0010010-E999","コントロールのイベント処理"));
			throw new FoundationException("AE0010010Control",e.getMessage(),"beginTransactionメソッドでエラーが発生しました。");
		}
