/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/DE0040/src/com/nec/jp/orteus/metamorBase/DE0040/DE0040B001/SQLCSCALCTRNDELETE.java,v $
 *
...
public class SQLCSCALCTRNDELETE extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public List call() throws BatchAppException
		{
			try {
				...
				cstmt.setString(9, m_med.getpnumCsCalcTrnPeropd());
				...
				if(rs != null) {
					...
					m_med.setpnumReturn(cstmt.getString(10));
					list.add(m_med);
				}
				...
				e.printStackTrace();
				BatchAppException be=new BatchAppException("SQLCSCALCTRNDELETE", "call", "Stored Procedure条件設定処理失敗");
				...
				if (cstmt != null) {
					try {
						...
						ef.printStackTrace();
						BatchAppException bef = new BatchAppException("SQLCSCALCTRNDELETE", "call", "Stored Procedureクローズ処理失敗");
		 ...
		 */
		public medDE0040B001 call(ResultSet rs) throws SQLException
		{
			if(rs == null || rs.next()==false) return null;
			medDE0040B001 data = new medDE0040B001();
			m_med.setpnumReturn(cstmt.getString(10));
			return data;
		}
	...
	/**
	 * SQLCSCALCTRNDELETEクラスの標準コンストラクタ
	 *
		 */
	public SQLCSCALCTRNDELETE(medDE0040B001 med, BatchAppConductor cdr) throws SystemException
	{
	...
	protected static String m_calcmd = "{" + "call SQLCSCALCTRNDELETE(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)" + "}";
