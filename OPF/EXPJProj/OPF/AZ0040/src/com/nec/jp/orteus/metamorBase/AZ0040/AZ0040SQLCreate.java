 ...
 * SQL文を組立てる
 */
public class AZ0040SQLCreate implements AZ0040Const
{
	//--------------------------------------------------------------------------
	// 構築
	...
	/** コンストラクタ */
	public AZ0040SQLCreate(SystemLog sysLog, String User_ID)
	{
		_sysLog = sysLog;
		_User_ID = User_ID;
	}
	 ...
	 * @param w_cdata AZ0040ColumnData 画面設定情報
	 * @param display_flg ＳＱＬ文字列作成条件 (0:表示用/1:ＤＢ登録用) 
	 * @return true:成功 / false:失敗
	 */
	public final void Create(IDbConnection conn,AZ0040ColumnData w_cdata,int display_flg) 
                            throws SQLException
	{
			int cnt = 0;
			String sSQL     = "";
			...
			{
				col_info = (AZ0040ColumnInfo)w_cdata.Column_infos.get(cnt);
				String w_colName = "";
				String checkFlg = "";
				...
				if(display_flg == C_DISPLAY) {
					...
					{
						for (int j=0; j < cnt; j++){
							if(col_info.getColumn_Name().equals(((AZ0040ColumnInfo)w_cdata.Column_infos.get(j)).getColumn_Name())){
								checkFlg="1";
								break;
							}else{
								checkFlg="0";
					...
					w_colName =  this.getColumnName(col_info,"");
					val += w_colName + ",";
				}
			}
			...
			for(cnt=0; cnt<sortList.size(); cnt++)
			{
				...
				col_info = (AZ0040ColumnInfo)w_cdata.Column_infos.get(index);
	 ...
	 * @return WHERE句
	 */
	private String MakesWhere(AZ0040ColumnInfo info)
	{
		String buf = null;
	...
	//  非公開関数
	//---------------------------------------
	private String getColumnName(AZ0040ColumnInfo i_info,String checkAlias) {
		String w_tableName = "";
		String w_columnName = "";
	...
	private String getColumnNameWhere(AZ0040ColumnInfo i_info,String strAlias) {
