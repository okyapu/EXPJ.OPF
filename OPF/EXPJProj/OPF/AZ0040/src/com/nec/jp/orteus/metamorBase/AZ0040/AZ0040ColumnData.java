//------------------------------------------------------------------------------
/**
 * CLASS     : AZ0040ColumnData クラス
 * ファイル・クラス説明
 * @author $Author: zhou-kehong $
 * @version $Revision: 1.2 $ $Date: 2015/04/07 11:58:21 $
 *
 */
//-----------------------------------------------------------------------------
...
//------------------------------------------------------------------
    ...
    public final AZ0040ColumnInfo  getColumnInfo(int Idx) { // カラム情報クラス
        return (AZ0040ColumnInfo)Column_infos.get(Idx);
    }
...
//------------------------------------------------------------------
	 ...
	 *
	*/
	AZ0040ColumnInfo getColumnInfo(String TableName, String ColumnName, int ColumnCount) {
		AZ0040ColumnInfo w_info = null;
		int checkCount = 0;
		for (Iterator itr=Column_infos.iterator(); itr.hasNext();) {
			AZ0040ColumnInfo col_info = (AZ0040ColumnInfo)itr.next();
			if(TableName != null) {
	 ...
	 * AZ0040SQLEdit.Analize()からsetColumnInfoWhere()経由で呼び出される。
	 *
	 */
	 ...
	 * AZ0040SQLEdit.setJoinInfo() から呼び出される
