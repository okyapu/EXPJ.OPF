/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/KV0040/src/com/nec/jp/orteus/metamorBase/KV0040/KV0040B001/callSQL.java,v $
 *
 ...
 * @version $Revision: 1.6 $ $Date: 2016/05/23 08:25:58 $
 */

//{{user_implement_dev:header
//}}user_implement_dev:header

public class callSQL extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public List call() throws BatchAppException
		{
			try {
				...
				cstmt.setString(8, m_med.getpvc2CompanyCd());
				cstmt.setString(9, m_med.getpnumYearMonth());


				cstmt.execute();

				List list = new ArrayList();
				medKV0040B001 data = null;
				...
				e.printStackTrace();
				BatchAppException be=new BatchAppException("callSQL", "call", "Stored Procedure条件設定処理失敗");
				...
				if (cstmt != null) {
					try {
						...
						ef.printStackTrace();
						BatchAppException bef = new BatchAppException("callSQL", "call", "Stored Procedureクローズ処理失敗");
		 ...
		 */
		public medKV0040B001 call(ResultSet rs) throws SQLException
		{
			if(rs == null || rs.next()==false) return null;
			medKV0040B001 data = new medKV0040B001();
			return data;
		}
	...
	/**
	 * callSQLクラスの標準コンストラクタ
	 *
		 */
	public callSQL(medKV0040B001 med, BatchAppConductor cdr) throws SystemException
	{
	...
	protected static String m_calcmd = "{" + "call SqlSalesTrnDelete(?, ?, ?, ?, ?, ?, ?, ?, ?)" + "}";
