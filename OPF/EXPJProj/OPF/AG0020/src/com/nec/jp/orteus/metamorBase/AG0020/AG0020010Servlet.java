...
public class AG0020010Servlet extends HttpServlet
{
	 ...
	 * @param control AG0020010Controlクラスインスタンス
	 */
					...
					AG0020010Control control ) throws BusinessProcessException, ServletException, IOException
	{
		...
		try {
			...
			if(("1").equals(choice) == true){
				struct.setw_CHOICE(choice);      //入庫
			}
			else if(("2").equals(choice) == true){
				struct.setw_CHOICE(choice);      //出庫
			}
			else{
				struct.setw_CHOICE(null);        //入出庫
			}
...
//			e.printStackTrace();
	...
	public static String DEFAULT_TARGET = "/AG0020010.jsp";          // 移動先URLの指定
	public static String JSP_BIND_NAME_Control = "aAG0020010Control"; // JSP先のバインド名(Control)の指定
	public static String JSP_BIND_NAME_Struct = "aAG0020010Struct";   // JSP先のバインド名(Struct)の指定
