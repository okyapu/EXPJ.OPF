/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/AD0110/src/com/nec/jp/orteus/metamorBase/AD0110/AD0110B001/SYS_ACTUAL_COST_CTRL_AC.java,v $
 *
...
/**
 * CLASS     : SYS_ACTUAL_COST_CTRL_AC クラス
 * ファイル・クラス説明
...
public class SYS_ACTUAL_COST_CTRL_AC extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("SYS_ACTUAL_COST_CTRL_AC", "read", "レコード検索条件設定処理失敗");
		...
		public String getPROC_EXEC_DATE_R() throws FoundationException { return m_doRs.getString(1); }
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				m_med.setPROC_EXEC_DATE(getPROC_EXEC_DATE_R());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("SYS_ACTUAL_COST_CTRL_AC", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("SYS_ACTUAL_COST_CTRL_AC", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	...
	/**
	 * SYS_ACTUAL_COST_CTRL_ACクラスの標準コンストラクタ
	 *
		 */
	public SYS_ACTUAL_COST_CTRL_AC(medAD0110B001 med, BatchAppConductor cdr) throws SystemException
	{
...
+"  TO_CHAR(TO_DATE(SYS_ACTUAL_COST_CTRL_AC.\"PROC_EXEC_DATE\", 'YYYY/MM'), 'YYYY/MM') AS \"PROC_EXEC_DATE\" "
+"FROM "
+"  SYS_ACTUAL_COST_CTRL_AC "
+"WHERE "
+"  SYS_ACTUAL_COST_CTRL_AC.\"PLANT_CD\" = ?";
