...
public class AZ0050DBUtil 
{
	...
	public AZ0050DBUtil(SystemLog sysLog, String User_ID) //
	{
		_sysLog = sysLog;
		_User_ID = User_ID;
	...
	{
		//_sysLog.finest_d("AZ0050DBUtil：load", _User_ID);
		...
		try {
			...
			for(int i=0; i< list.size(); i++)
			{
				...
				AZ0050DBInfo l_COMMENT = new AZ0050DBInfo();
	...
	/**
	 * AZ0050DBInfoクラスの取得<br>
	 * <br>
	 * @param tablename テーブル名
	 * @return AZ0050DBInfoクラス 該当するテーブルが見つからない場合 null
	 */
	public AZ0050DBInfo getDBInfo(String tableName)
	{
		if(tableName == null)  return null;

		return (AZ0050DBInfo)_infoMap.get(tableName);
	 ...
	 * 
	 */
	// AZ0050DBUtil()コンストラクタ呼び出しを前提とする。
	// (テーブル選択画面のテーブル一覧用)
	...
	{
		if(tableName == null)  return null;
		AZ0050DBInfo info = this.getDBInfo(tableName);	//該当するDBInfoを取り出す
	...
	public String getColumnComment(String tableName, String columnName)
	{
		...
		AZ0050DBInfo info = this.getDBInfo(tableName);
		if(info == null) return null;
