              ...
              implements AZ0050Const
{
	...
	public AZ0050SQLEdit(SystemLog i_syslog, String i_userid){
	    //SystemLog用
		_sysLog = i_syslog;
    	_UserId = i_userid;
                                                 ...
                                                 throws SQLException
	{
		try
		{
			...
			{
				AZ0050ColumnInfo col_info = (AZ0050ColumnInfo)_w_cdata.Column_infos.get(cnt);
...
//***************************************************
	 ...
	 * @return AZ0050ColumnData  解析したSQLデータ
	 */
	public AZ0050ColumnData getColumnData()
	{
		return _w_cdata;
	}
	 ...
	 * (DBUtilクラス)を取得する
	 * @return AZ0050DBUtil 解析したSQLデータ
	 */
	public final AZ0050DBUtil getDBUtil() {
		return _dbutil;
	}
	...
	protected final boolean renewColumnInfos(List l_table, List l_talias, List l_column)
	{
		...
		for(int i=0; i<l_table.size(); i++) {
			//新しいColumnInfo 生成
			AZ0050ColumnInfo new_info = new AZ0050ColumnInfo();
			...
			//元の項目情報リストにあるかチェック
			AZ0050ColumnInfo org_info = _w_cdata.getColumnInfo(w_tableAlias, w_columnName, count);
		...
		for(Iterator itr=new_INFOS.iterator(); itr.hasNext();) {
			_w_cdata.Column_infos.add((AZ0050ColumnInfo)itr.next());
		}
		new_INFOS.clear();
		return true;
	}
...
//***************************************************
	...
	private void seColumnSet(String val)
	{
		...
		AZ0050ColumnInfo w_cinfo = new AZ0050ColumnInfo();
	...
	private void setAllColumns(String table)
	{
		...
		AZ0050DBInfo dbinfo = _dbutil.getDBInfo(table);
	 ...
	 * @return なし
	 */
	private void setColumnInfo(AZ0050ColumnInfo col_info)
	{
		int i = 0;
		String buffer = null;
	...
	private void setColumnInfoOrderBy(List valList)
	{
		String table = null;
		String column = null;
		AZ0050ColumnInfo col_info = null;
    ...
    /**
     * comment SQL(WHERE句)を解析し、結果をAZ0050ColumnDataに
     *         格納する。
     * @param 
     */
    ...
    */
	...
	/**
	 * 各カラムの検索条件をAZ0050ColumnDataに設定する
    ...
    */
	...
	private void clear()
	{ 
		_w_cdata = new AZ0050ColumnData();
		_dbutil = new AZ0050DBUtil(_sysLog,_UserId);
	}
	...
	private AZ0050ColumnData _w_cdata;		
	private AZ0050DBUtil _dbutil;			
