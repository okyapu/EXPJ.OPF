 ...
 * @version $Revision: 1.9 $ $Date: 2013/09/12 01:58:40 $
 *
 */
//------------------------------------------------------------------------------
...
public class AZ0050Common
{
	...
	public final static ArrayList GetUsefulTable(IDbConnection conn) throws SQLException
	{
		...
		String sql = "SELECT USER_TABLES.TABLE_NAME AS TABLE_NAME, " +
				"USER_TAB_COMMENTS.COMMENTS AS TABLE_COMMENTS " +
				"FROM USER_TABLES, USER_TAB_COMMENTS " +
				"WHERE USER_TABLES.TABLE_NAME = USER_TAB_COMMENTS.TABLE_NAME " +
				"UNION " +
				"(SELECT USER_VIEWS.VIEW_NAME AS TABLE_NAME, " +
				"USER_TAB_COMMENTS.COMMENTS AS TABLE_COMMENTS " +
				"FROM USER_VIEWS, USER_TAB_COMMENTS " +
				"WHERE USER_VIEWS.VIEW_NAME = USER_TAB_COMMENTS.TABLE_NAME) " +
    ...
    */
	public final static String EscapeSQL(String i_str) {
		...
		for (int i=0; i<w_str.length; i++) {
			//各値の最初と最後に、''をつける
			w_str_o.add("'" + toSqlString(w_str[i]) + "'"); 
		}
        Iterator itr = w_str_o.iterator();
    ...
    */
	public final static String EscapeSQL_Date(String i_str) {
		...
		for (int i=0; i<w_str.length; i++) {
			//各値にTO_DATEをつける
			if ("true".equals(time_ctrl)){
				w_str_o.add("TO_DATE('" + w_str[i] + "', '" + DATE_FORMAT + "')");
			} else {
				w_str_o.add("TRUNC(TO_DATE('" + w_str[i]  + "', '" + DATE_FORMAT + "'))");
			}
		}
        Iterator itr = w_str_o.iterator();
		String w_escape = StringUtils.join(itr, ",");
