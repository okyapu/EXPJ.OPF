/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/DB0020/src/com/nec/jp/orteus/metamorBase/DB0020/DB0020B001/SQLCSPUCHOUT.java,v $
 *
...
public class SQLCSPUCHOUT extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public List call() throws BatchAppException
		{
			try {
				...
				cstmt.setInt(8, m_med.getpnumYear().intValue());
				cstmt.setInt(9, m_med.getpnumHalfTermTyp().intValue());
				cstmt.setInt(10, m_med.getpnumCostTyp().intValue());
				cstmt.setInt(11, m_med.getpnumYyyyMm().intValue());
				cstmt.setInt(12, m_med.getpnumExecuteTyp().intValue());
				cstmt.setInt(13, m_med.getpnumUpdateMode().intValue());
				...
				if(rs != null) {
					...
					m_med.setpnumReturn( new Integer(cstmt.getInt(14)) );
					list.add(m_med);
				}
				...
				e.printStackTrace();
				BatchAppException be=new BatchAppException("SQLCSPUCHOUT", "call", "Stored Procedure条件設定処理失敗");
				...
				if (cstmt != null) {
					try {
						...
						ef.printStackTrace();
						BatchAppException bef = new BatchAppException("SQLCSPUCHOUT", "call", "Stored Procedureクローズ処理失敗");
		 ...
		 */
		public medDB0020B001 call(ResultSet rs) throws SQLException
		{
			if(rs == null || rs.next()==false) return null;
			medDB0020B001 data = new medDB0020B001();
			m_med.setpnumReturn( new Integer(cstmt.getInt(14)) );
			return data;
		}
	...
	/**
	 * SQLCSPUCHOUTクラスの標準コンストラクタ
	 *
		 */
	public SQLCSPUCHOUT(medDB0020B001 med, BatchAppConductor cdr) throws SystemException
	{
	...
	protected static String m_calcmd = "{" + "call SQLCSPUCHOUT(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)" + "}";
