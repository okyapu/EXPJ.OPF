/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/DB0020/src/com/nec/jp/orteus/metamorBase/DB0020/DB0020B001/SEQ_CS_ERROR_CD.java,v $
 *
...
public class SEQ_CS_ERROR_CD extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("SEQ_CS_ERROR_CD", "read", "レコード検索条件設定処理失敗");
		...
		public String getCS_ERROR_CD_R() throws FoundationException { return m_doRs.getString(1); }
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				m_med.setCS_ERROR_CD(getCS_ERROR_CD_R());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("SEQ_CS_ERROR_CD", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("SEQ_CS_ERROR_CD", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	...
	/**
	 * SEQ_CS_ERROR_CDクラスの標準コンストラクタ
	 *
		 */
	public SEQ_CS_ERROR_CD(medDB0020B001 med, BatchAppConductor cdr) throws SystemException
	{
