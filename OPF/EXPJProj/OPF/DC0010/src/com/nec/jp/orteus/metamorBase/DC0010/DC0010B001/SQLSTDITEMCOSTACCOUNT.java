/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/DC0010/src/com/nec/jp/orteus/metamorBase/DC0010/DC0010B001/SQLSTDITEMCOSTACCOUNT.java,v $
 *
...
/**
 * CLASS     : SQLSTDITEMCOSTACCOUNT クラス
 * ファイル・クラス説明
...
public class SQLSTDITEMCOSTACCOUNT extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public List call() throws BatchAppException
		{
			try {
				...
				e.printStackTrace();
				BatchAppException be=new BatchAppException("SQLSTDITEMCOSTACCOUNT", "call", "Stored Procedure条件設定処理失敗");
				...
				if (cstmt != null) {
					try {
						...
						ef.printStackTrace();
						BatchAppException bef = new BatchAppException("SQLSTDITEMCOSTACCOUNT", "call", "Stored Procedureクローズ処理失敗");
	...
	/**
	 * SQLSTDITEMCOSTACCOUNTクラスの標準コンストラクタ
	 *
		 */
	public SQLSTDITEMCOSTACCOUNT(medDC0010B001 med, BatchAppConductor cdr) throws SystemException
	{
	...
	protected static String m_calcmd = "{" + "call SQLSTDITEMCOSTACCOUNT(?, ?, ?, ?, ?, ?, ?, to_number(?), to_number(?), to_number(?), to_number(?), ?)" + "}";
