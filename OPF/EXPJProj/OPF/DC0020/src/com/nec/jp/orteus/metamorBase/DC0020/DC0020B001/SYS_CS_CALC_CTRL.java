/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/DC0020/src/com/nec/jp/orteus/metamorBase/DC0020/DC0020B001/SYS_CS_CALC_CTRL.java,v $
 *
...
public class SYS_CS_CALC_CTRL extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("SYS_CS_CALC_CTRL", "read", "レコード検索条件設定処理失敗");
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("SYS_CS_CALC_CTRL", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("SYS_CS_CALC_CTRL", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	 ...
	 *
		 */
	public SYS_CS_CALC_CTRL(medDC0020B001 med, BatchAppConductor cdr) throws SystemException
	{
...
+"  SYS_CS_CALC_CTRL.\"PLANT_CD\" as \"PLANT_CD\" "
+"from "
+"  SYS_CS_CALC_CTRL "
+"where "
