/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/DC0020/src/com/nec/jp/orteus/metamorBase/DC0020/DC0020B001/CtrlMain.java,v $
 *
 ...
 * @version $Revision: 1.8 $ $Date: 2014/11/05 07:23:15 $
 */
		...
		implements IBatchAppCommon
{
	 ...
	 *
		 */
	public CtrlMain(medDC0020B001 med, BatchAppConductor cdr) throws BatchAppException
	{
		// メンバ変数を初期化します
		m_conductor = cdr;
		m_med = med;
	...
	//{{user_implement_code
         ...
         *
         */
        public CtrlMain(medDC0020B001 med) throws BatchAppException {
         // メンバ変数を初期化します
         m_med = med;
         m_control = new interControl(med);
        ...
        /** PL/SQL 標準材料費・加工費計算処理(CHECKのみ) を表す定数 */
        static final int SQLSTDMATRPROCCHECK = 6;
        ...
        public int main() throws BatchAppException {
         ...
         try {
          ...
          // 標準原価材料費・加工費計算処理（チェックのみ）
          result = executeProcedure(SQLSTDMATRPROCCHECK);
          System.out.println("result:" + result);
          if ("3".equals(result)) {
           // [原価計算処理制御]を終了状態として更新
           m_med.getSYS_CS_CALC_CTRL_Stop().update();
         ...
         * @throws Exception
         */
        private String executeProcedure(int csCalcProcNo) throws BatchAppException, FoundationException {
         // [バッチパラメータ]の検索
         m_med.setPROGRAM_CD(m_med.getProgname());
         if (csCalcProcNo == SQLSTDITEMPSWORKENTRY) {
          m_med.setFUNCTION_NAME("SQLSTDITEMPSWORKENTRY");
         } else if (csCalcProcNo == SQLSTDMATRPROCCHECK) {
          m_med.setFUNCTION_NAME("SQLSTDMATRPROCCHECK");
         }
         ...
         if (csCalcProcNo == SQLSTDITEMPSWORKENTRY) {
          ...
          callList = m_med.getSQLSTDMATRPROCCHECK().call();
         }
       
         // PL/SQLの実行結果を取得
         medDC0020B001 resultMed = (medDC0020B001) callList.get(0);
		...
		implements IBatchAppCommon
	{
		...
		public interControl(medDC0020B001 med) throws BatchAppException
		{
			// 各メンバ変数の初期化
			m_med = med;
