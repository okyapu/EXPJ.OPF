/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/DC0020/src/com/nec/jp/orteus/metamorBase/DC0020/DC0020B001/SQLSTDITEMPSWORKENTRY.java,v $
 *
...
/**
 * CLASS     : SQLSTDITEMPSWORKENTRY クラス
 * ファイル・クラス説明
 * @author $Author: xl-hs-zzw $
 * @version $Revision: 1.6 $ $Date: 2014/11/05 07:23:15 $
 */

//{{user_implement_dev:header
//}}user_implement_dev:header

public class SQLSTDITEMPSWORKENTRY extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public List call() throws BatchAppException
		{
			try {
				...
				cstmt.setString(8, m_med.getpnumYear());
				cstmt.setString(9, m_med.getpnumHalfTermTyp());
				cstmt.setString(10, m_med.getpnumCostTyp());
				cstmt.setString(11, m_med.getpnumActTyp());
				...
				e.printStackTrace();
				BatchAppException be=new BatchAppException("SQLSTDITEMPSWORKENTRY", "call", "Stored Procedure条件設定処理失敗");
				...
				if (cstmt != null) {
					try {
						...
						ef.printStackTrace();
						BatchAppException bef = new BatchAppException("SQLSTDITEMPSWORKENTRY", "call", "Stored Procedureクローズ処理失敗");
		 ...
		 */
		public medDC0020B001 call(ResultSet rs) throws SQLException
		{
			if(rs == null || rs.next()==false) return null;
			medDC0020B001 data = new medDC0020B001();
			m_med.setpnumReturn(cstmt.getString(12));
			return data;
		}
	...
	/**
	 * SQLSTDITEMPSWORKENTRYクラスの標準コンストラクタ
	 *
		 */
	public SQLSTDITEMPSWORKENTRY(medDC0020B001 med, BatchAppConductor cdr) throws SystemException
	{
	...
	protected static String m_calcmd = "{" + "call SQLSTDITEMPSWORKENTRY(?, ?, ?, ?, ?, ?, ?, to_number(?), to_number(?), to_number(?), to_number(?), ?)" + "}";
