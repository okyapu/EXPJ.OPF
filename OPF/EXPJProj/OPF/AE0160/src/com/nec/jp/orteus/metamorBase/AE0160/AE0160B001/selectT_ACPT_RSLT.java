/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/AE0160/src/com/nec/jp/orteus/metamorBase/AE0160/AE0160B001/selectT_ACPT_RSLT.java,v $
 *
...
public class selectT_ACPT_RSLT extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				setIN1_R(m_med.getTEMP_PUCH_ODR_CD());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("selectT_ACPT_RSLT", "read", "レコード検索条件設定処理失敗");
		...
		public String getACPT_QTY_R() throws FoundationException { return m_doRs.getString(1); }
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("selectT_ACPT_RSLT", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("selectT_ACPT_RSLT", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	...
	/**
	 * selectT_ACPT_RSLTクラスの標準コンストラクタ
	 *
		 */
	public selectT_ACPT_RSLT(medAE0160B001 med, BatchAppConductor cdr) throws SystemException
	{
...
+"  T.PUCH_ODR_QTY - NVL(ACPT_RSLT.ACPT_QTY, 0) AS ACPT_QTY "
+"FROM "
+"  T_RLSD_PUCH_ODR T, "
+"  ( "
+"    SELECT "
+"      SUM(M.ACPT_QTY) AS ACPT_QTY, "
+"      M.PUCH_ODR_CD "
+"    FROM "
+"      T_ACPT_RSLT M "
+"    GROUP BY "
...
+"WHERE "
+"  T.PUCH_ODR_CD = ? "
+"  AND T.PUCH_ODR_CD = ACPT_RSLT.PUCH_ODR_CD(+)";
