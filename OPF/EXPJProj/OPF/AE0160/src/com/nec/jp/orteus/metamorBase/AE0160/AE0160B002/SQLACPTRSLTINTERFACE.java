/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/AE0160/src/com/nec/jp/orteus/metamorBase/AE0160/AE0160B002/SQLACPTRSLTINTERFACE.java,v $
 *
...
/**
 * CLASS     : SQLACPTRSLTINTERFACE クラス
 * ファイル・クラス説明
...
public class SQLACPTRSLTINTERFACE extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public List call() throws BatchAppException
		{
			try {
				...
				cstmt.setString(8, m_med.getpnumCsvTakeTyp());
				cstmt.setString(9, m_med.getpvc2CsvTakePath());
				cstmt.setString(10, m_med.getpvc2CsvBackupPath());
				...
				if(rs != null) {
					...
					m_med.setpnumReturn(cstmt.getString(11));
					list.add(m_med);
				}
				...
				e.printStackTrace();
				BatchAppException be=new BatchAppException("SQLACPTRSLTINTERFACE", "call", "Stored Procedure条件設定処理失敗");
				...
				if (cstmt != null) {
					try {
						...
						ef.printStackTrace();
						BatchAppException bef = new BatchAppException("SQLACPTRSLTINTERFACE", "call", "Stored Procedureクローズ処理失敗");
		 ...
		 */
		public medAE0160B002 call(ResultSet rs) throws SQLException
		{
			if(rs == null || rs.next()==false) return null;
			medAE0160B002 data = new medAE0160B002();
			m_med.setpnumReturn(cstmt.getString(11));
			return data;
		}
	...
	/**
	 * SQLACPTRSLTINTERFACEクラスの標準コンストラクタ
	 *
		 */
	public SQLACPTRSLTINTERFACE(medAE0160B002 med, BatchAppConductor cdr) throws SystemException
	{
	...
	protected static String m_calcmd = "{" + "Call SQLACPTRSLTINTERFACE(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)" + "}";
