/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/AE0160/src/com/nec/jp/orteus/metamorBase/AE0160/AE0160B001/lockT_ACPT_RSLT_TEMP.java,v $
 *
...
/**
 * CLASS     : lockT_ACPT_RSLT_TEMP クラス
 * ファイル・クラス説明
...
public class lockT_ACPT_RSLT_TEMP extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				setIN1_R(m_med.getTEMP_ACPT_RSLT_TEMP_CTL_NO());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("lockT_ACPT_RSLT_TEMP", "read", "レコード検索条件設定処理失敗");
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("lockT_ACPT_RSLT_TEMP", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("lockT_ACPT_RSLT_TEMP", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	...
	/**
	 * lockT_ACPT_RSLT_TEMPクラスの標準コンストラクタ
	 *
		 */
	public lockT_ACPT_RSLT_TEMP(medAE0160B001 med, BatchAppConductor cdr) throws SystemException
	{
