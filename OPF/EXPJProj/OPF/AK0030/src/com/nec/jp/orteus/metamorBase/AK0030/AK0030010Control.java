...
public class AK0030010Control
{
	...
	//{{user_implement_code_main
       ...
       //	public Object getListvalue(int x) { return this.list.get(x); }	// リストの値を返します。
        public AK0030010Struct getListvalue(int x) { return (AK0030010Struct)(this.list.get(x)); }	// リストの値を返します。
        public AK0030010Struct getStruct() { return this.struct; }	// Structを返します。
        public AK0030010Struct createStruct() { return new AK0030010Struct(); }	// 新しいStructを作って返します。
        public void setStruct(Object structname) { this.struct.setStruct((AK0030010Struct)structname); }	// Structをセットします。
        ...
        public boolean initialSet() {
         try {
          ...
          if (prdReqCtrlList == null || prdReqCtrlList.size() == 0) {
           setErrorMessage("ZZ01101"); // 指定キーのデータが存在しません。
          ...
          this.struct.setPRD_REQ_START_DATE(
            selectStruct.getPRD_REQ_START_DATE());    // 生産要求処理開始日時
          
          itemConvEndDate 
            = selectStruct.getITEM_CONV_END_DATE();   // 生産要求生成処理終了日時
          mrpDmEndDate 
            = selectStruct.getMRP_DM_END_DATE();      // MRP品デマンド生成処理終了日
          shipConvEndDate 
            = selectStruct.getSHIP_CONV_END_DATE();   // 出荷商品品目変換処理終了日
          stockIssueEndDate 
            = selectStruct.getSTOCK_ISSUE_END_DATE(); // 在庫出庫処理終了日時
          
          // Radioボタン位置設定（posのセット）
          ...
          setErrorMessage("ZZ01106"); // サーバでエラーが発生しました。
          return false;
         }
         return true;
        }
		...
		logger.entering("AK0030010Control"+":USER="+struct.getsUser_ID(),"controlExecute");
			//{{user_implement_dev:<controlExecute>
                ...
                try{
                 ...
                 String prdReqStartDate = null;     // 前回実行日
                 ...
                 prdReqStartDate = this.struct.getPRD_REQ_START_DATE(); // 前回実行日
...
//                 cmd.append(" ");
//                 cmd.append(plantCd);             // 工場コード
//                 cmd.append(" ");
//                 cmd.append(stepFlg);             // ステップフラグ
//                 
//                 // バッチコマンドを実行
...
//                 Process p = runtime.exec(cmd.toString());
                 ...
                 buffer.add("com.nec.jp.orteus.metamorBase.AK0030.AK0030B001.mainAK0030B001");		// 呼び出し先クラス
                 buffer.add(batchProcessingTyp);		// バッチ処理区分
                 buffer.add(userId);					// ユーザID
                 buffer.add(plantCd);					// 工場コード
                 buffer.add(stepFlg);					// ステップフラグ
                 // バッチコマンドを実行
                 Runtime runtime = Runtime.getRuntime();
...
//			e.printStackTrace();
//			throw new FoundationException("AK0030010Control",e.getMessage(),"control_eventHandllerメソッドでエラーが発生しました。");
	...
	public void createDefaultCSV(List list, String sTitle[]) throws BusinessProcessException, FoundationException {
		...
		//{{user_implement_dev:<createDefaultCSV>
			...
			e.printStackTrace();
//			objMessage.m_writer.write(Level.ALL, objMessage.getIDMsg("AK0030010-E999","CSVの出力処理"));
			throw new FoundationException("AK0030010Control",e.getMessage(),"createDefaultCSVメソッドでエラーが発生しました。");
		}
...
//				objMessage.m_writer.write(Level.ALL, objMessage.getIDMsg("AK0030010-E999","システム日付の取得処理"));
				throw new FoundationException("AK0030010Control","beginTransaction()","システム日付取得処理でエラーが発生しました。");
			}
		...
		//{{user_implement_dev:<beginTransaction>
			...
			e.printStackTrace();
//			objMessage.m_writer.write(Level.ALL, objMessage.getIDMsg("AK0030010-E999","コントロールのイベント処理"));
			throw new FoundationException("AK0030010Control",e.getMessage(),"beginTransactionメソッドでエラーが発生しました。");
		}
