/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/AL0030/src/com/nec/jp/orteus/metamorBase/AL0030/AL0030B001/CtrlMain.java,v $
 *
 ...
 * @version $Revision: 1.5 $ $Date: 2014/11/05 07:19:34 $
 */
		...
		implements IBatchAppCommon
{
	 ...
	 *
		 */
	public CtrlMain(medAL0030B001 med, BatchAppConductor cdr) throws BatchAppException
	{
		// メンバ変数を初期化します
		m_conductor = cdr;
		m_med = med;
	...
	//{{user_implement_code
            ...
            * @return なし
            */
           public CtrlMain(medAL0030B001 med) throws BatchAppException
           {
               // メンバ変数を初期化します
               m_med = med;
               m_control = new interControl(med);
           ...
           //ファンクション名
           functionName = "SQLLOADPSMAKE"; //負荷構成マスタ作成
           ...
           try{
               try{
                   ...
                   // 2003.11.06 コマンド用　引数に「null」が入っていた場合チェック
                   ...
                   try {
                       ...
                       // lockCancelKey==-1の場合業務ロックされている為エラー
                       if(lockCancelKey == -1){
       ...
       emsg = new ExpjMessage("ZZ01006", myBusinessName);
       m_med.getSyslog().warning(emsg, "test");
       emsg = new ExpjMessage("ZZ01006", functionName);
       m_med.getSyslog().warning(emsg, "test");
       //!-- DEBUG end --
                   ...
                   //ストアドプロシージャ実行
                   try {
                       m_med.getcallSqlLoadPsMake().call();
                   } catch (Exception e) {
                   ...
                   try {
                       ...
                       if(businessErrCode == null) {
                           Lock.doUnLock(myBusinessName, lockCancelKey);
                           conn.commit();
                       }
                   } catch (FoundationException e) {
                       e.printStackTrace();
		...
		implements IBatchAppCommon
	{
		...
		public interControl(medAL0030B001 med) throws BatchAppException
		{
			// 各メンバ変数の初期化
			m_med = med;
