/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/AK0010/src/com/nec/jp/orteus/metamorBase/AK0010/AK0010B001/callSqlDailyProc.java,v $
 *
...
public class callSqlDailyProc extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public List call() throws BatchAppException
		{
			try {
				...
				cstmt.setString(8, m_med.getpvc2FinalDailyProcExecDate());
				cstmt.setString(9, m_med.getpvc2BusinessOprDate());


				cstmt.execute();

				List list = new ArrayList();
				medAK0010B001 data = null;
				...
				e.printStackTrace();
				BatchAppException be=new BatchAppException("callSqlDailyProc", "call", "Stored Procedure条件設定処理失敗");
				...
				if (cstmt != null) {
					try {
						...
						ef.printStackTrace();
						BatchAppException bef = new BatchAppException("callSqlDailyProc", "call", "Stored Procedureクローズ処理失敗");
		 ...
		 */
		public medAK0010B001 call(ResultSet rs) throws SQLException
		{
			if(rs == null || rs.next()==false) return null;
			medAK0010B001 data = new medAK0010B001();
			return data;
		}
	...
	/**
	 * callSqlDailyProcクラスの標準コンストラクタ
	 *
		 */
	public callSqlDailyProc(medAK0010B001 med, BatchAppConductor cdr) throws SystemException
	{
	...
	protected static String m_calcmd = "{" + "call SqlDailyProc(?,?,?,?,?,?,?,?,?)" + "}";
