/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/AK0010/src/com/nec/jp/orteus/metamorBase/AK0010/AK0010B001/checkPlantCd.java,v $
 *
...
public class checkPlantCd extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("checkPlantCd", "read", "レコード検索条件設定処理失敗");
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				m_med.setchPlantCd(getPLANT_CD_R());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("checkPlantCd", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("checkPlantCd", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	...
	/**
	 * checkPlantCdクラスの標準コンストラクタ
	 *
		 */
	public checkPlantCd(medAK0010B001 med, BatchAppConductor cdr) throws SystemException
	{
