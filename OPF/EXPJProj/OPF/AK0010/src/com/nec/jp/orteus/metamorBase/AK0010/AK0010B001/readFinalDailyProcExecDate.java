/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/AK0010/src/com/nec/jp/orteus/metamorBase/AK0010/AK0010B001/readFinalDailyProcExecDate.java,v $
 *
...
/**
 * CLASS     : readFinalDailyProcExecDate クラス
 * ファイル・クラス説明
...
public class readFinalDailyProcExecDate extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				setIN2_R(m_med.getfinalPlantCd());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("readFinalDailyProcExecDate", "read", "レコード検索条件設定処理失敗");
		...
		public String getFINAL_DAILY_PROC_EXEC_DATE_R() throws FoundationException { return m_doRs.getString(1); }
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				m_med.setfinalDailyProcExecDate(getFINAL_DAILY_PROC_EXEC_DATE_R());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("readFinalDailyProcExecDate", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("readFinalDailyProcExecDate", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	 ...
	 *
		 */
	public readFinalDailyProcExecDate(medAK0010B001 med, BatchAppConductor cdr) throws SystemException
	{
