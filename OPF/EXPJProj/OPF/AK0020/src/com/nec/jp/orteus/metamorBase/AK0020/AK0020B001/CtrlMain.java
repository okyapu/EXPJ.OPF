/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/AK0020/src/com/nec/jp/orteus/metamorBase/AK0020/AK0020B001/CtrlMain.java,v $
 *
 ...
 * @version $Revision: 1.9 $ $Date: 2016/05/23 08:27:46 $
 */
		...
		implements IBatchAppCommon
{
	 ...
	 *
		 */
	public CtrlMain(medAK0020B001 med, BatchAppConductor cdr) throws BatchAppException
	{
		// メンバ変数を初期化します
		m_conductor = cdr;
		m_med = med;
	...
	//{{user_implement_code
        ...
        public boolean checkFinalMonthlyDate(String plantCd, String businessName,
         ...
         try {
          ...
          if (dateScreenMonthlyDate.compareTo(dateSelectMonthlyDate) < 0) {
           ...
           errCodeList.add("AK33261"); //過去の月次処理は実行できません。
           setErrNote("(SBM0631)工場コード：[ " + plantCd 
             + " ] 月次処理対象年月：[ " + finalMonthlyDate 
             + " ] 最終月次処理済年月：[ " + selectmonthlydate + " ]" );
           setErrPlantCd(plantCd);
           emsg = new ExpjMessage("ZZ01006", 
             "(SBM0954)「月次処理対象年月」＜[月次処理制御].最終月次処理済年月エラー");
           m_med.getSyslog().severe(emsg, userId);
           return false;
          } else if (dateScreenMonthlyDate.compareTo(dateSelectMonthlyDate) == 0) {
           //「月次処理対象年月」＝［月次処理制御］."最終月次処理済年月"ワーニング
           errCodeList.add("AK00019"); //処理済の年月で再度処理を行いました。
           setErrNote("(SBM0632)月次処理対象年月：[ " + finalMonthlyDate 
             + " ] 最終月次処理済年月：[ " + selectmonthlydate + " ]");
           setErrPlantCd(plantCd);
           emsg = new ExpjMessage("ZZ01006", 
             "(SBM0955)「月次処理対象年月」＝［月次処理制御］.最終月次処理済年月ワーニング");
           m_med.getSyslog().severe(emsg, userId);
          }
         } catch(Exception e) {
          errCodeList.add("ZZ01106");// サーバでエラーが発生しました。
        ...
        public boolean spExecute(String logMode, String outputMode, String outputPath, 
         ...
         try {
          ...
          m_med.setpvc2MonthlyDate(finalMonthlyDate);
          ...
          try {
           ...
           m_med.getSyslog().info(emsg, userId);
           m_med.getcallSqlMonthlyProc().call();
          } catch (Exception e) {
         ...
         *
         */
        public CtrlMain(medAK0020B001 med) throws BatchAppException
        {
         // メンバ変数を初期化します
         m_med = med;
         m_control = new interControl(med);
       ...
       */
         ...
         // ストアドプロシージャ名
         String functionName = "SQLMONTHLYPROC";  // 月次処理
         ...
         String finalMonthlyDate = null;    // 画面の月次処理対象日
       ...
       // 引数の妥当性をチェックし、変数にセットする -----------------------------------------------------
          ...
          plantCd = args[2];
          if (argsLength == 4) {
           finalMonthlyDate = args[3];    // 画面の月次処理対象日
          }
          ...
          if (batchProcessingTyp != 1 || argsLength != 3) {
           if (finalMonthlyDate == null || finalMonthlyDate.equals("null") 
            ...
            errCodeList.add("AK00006"); // バッチ処理引数の形式が不正です
            setErrNote("(SBM0633)月次処理対象日：[ " + finalMonthlyDate + " ]");
            setErrPlantCd(plantCd);
            Exception e = new Exception();
            emsg = new ExpjMessage("ZZ01006", "(SBM0959)月次処理対象日が未入力です");
            m_med.getSyslog().severe(emsg, userId);
            throw e;
           }
          }
       ...
       // 入力チェック************************************************************************************
          ...
          if ( batchProcessingTyp == 1 ) {
           ...
           if (argsLength != 3) {
            ...
            try {
             ...
             errCodeList.add("AK00018");// 月次処理対象日の形式が不正です。
             setErrNote("(SBM0633)月次処理対象日：[ " + finalMonthlyDate + " ]");
             setErrPlantCd(plantCd);
             emsg = new ExpjMessage("ZZ01006", "(SBM0960)月次処理対象日の形式が不正です。");
             m_med.getSyslog().severe(emsg, userId);
             throw e;
            }
           }
          ...
          if (batchProcessingTyp == 1 && argsLength == 3) {
           ...
           m_med.setprogramCd(businessName);
           m_med.setfunctionName(functionName);
           AbstractBatchAppEntity selectfinalMonthlyDate 
             = m_med.getselectFinalMonthlyDate();
           try {
            selectfinalMonthlyDate.read();
           } catch (Exception e) {
            errCodeList.add("ZZ01106");// サーバでエラーが発生しました。
           ...
           //Date dateNextMonth = sdf.parse(selectmonthlydate);        // Date型に変換
           Date dateNextMonth = Converter.strToDate(selectmonthlydate, "yyyy/MM"); // Date型に変換
           ...
           finalMonthlyDate = nextDateStr;    // 画面の月次処理対象日
          }
          ...
          if (!spExecute(logMode, outputMode, outputPath, outputName, 
            userId, businessName, plantCd, finalMonthlyDate)) {
		...
		implements IBatchAppCommon
	{
		...
		public interControl(medAK0020B001 med) throws BatchAppException
		{
			// 各メンバ変数の初期化
			m_med = med;
