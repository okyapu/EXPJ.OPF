/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/DB0030/src/com/nec/jp/orteus/metamorBase/DB0030/DB0030010Control.java,v $
 *
...
import com.nec.jp.orteus.metamorBase.DB0030.*;
...
public class DB0030010Control
{
	...
	public DB0030010Struct getListvalue(int x) {
		return (DB0030010Struct) (this.list.get(x));
	} // リストの値を返します。
	public DB0030010Struct getStruct() {
		return this.struct;
	} // Structを返します。
	public DB0030010Struct createStruct() {
		return new DB0030010Struct();
	} // 新しいStructを作って返します。
	public void setStruct(Object structname) {
		this.struct.setStruct((DB0030010Struct) structname);
	} // Structをセットします。
	...
	private DB0030010Struct _mySysCostCtrl;
	 ...
	 * @param s 検索したデータ
	 */
	private void setStruct(DB0030010Struct s) {
	}
	 ...
	 *          3:工程情報一括登録のみ
	 */
	private String getExecuteType(DB0030010Struct s) {
	 ...
	 *          2:全件更新
	 */
	private String getUpdateMode(DB0030010Struct s) {
	 ...
	 */
	private void executeBatchProgram(DB0030010Struct s) throws IOException {
...
//		// 呼び出し先クラス
//		buffer.append(" com.nec.jp.orteus.metamorBase.DB0030.DB0030B001.mainDB0030B001");
//		// バッチ処理区分
...
//		runtime.exec(buffer.toString());
		...
		// 呼び出し先クラス
		buffer.add("com.nec.jp.orteus.metamorBase.DB0030.DB0030B001.mainDB0030B001");
		// バッチ処理区分
		...
		// 原価種別
		buffer.add(s.getCOST_TYP());
		// 実行処理区分	
		buffer.add(getExecuteType(s));
		// 更新モード
		buffer.add(getUpdateMode(s));
		// バッチコマンドを実行
		Runtime runtime = Runtime.getRuntime();
	...
	{
		Logger logger = Logger.getLogger("com.nec.jp.orteus.metamorBase.DB0030");
		logger.entering("DB0030010Control"+":USER="+struct.getsUser_ID(),"controlExecute");
		...
		try {
			...
			if (mPlantList.isEmpty()) {
				...
				DB0030010Struct plantStruct = (DB0030010Struct) mPlantList.get(0);
			...
			if (sysCostCtrlList.isEmpty()) {
				...
				DB0030010Struct sysCostCtrlStruct = (DB0030010Struct) sysCostCtrlList.get(0);
			...
			setInformationMessage("ZZ09016");
                ...
                //}}user_implement_dev:<controlExecute>
		logger.exiting("DB0030010Control"+":USER="+struct.getsUser_ID(),"controlExecute");
	...
	{
		Logger logger = Logger.getLogger("com.nec.jp.orteus.metamorBase.DB0030");
		logger.entering("DB0030010Control"+":USER="+struct.getsUser_ID(),"controlClear");
			//{{user_implement_dev:<controlClear>
		initializeAll();
                //}}user_implement_dev:<controlClear>
		logger.exiting("DB0030010Control"+":USER="+struct.getsUser_ID(),"controlClear");
	...
	{
		Logger logger = Logger.getLogger("com.nec.jp.orteus.metamorBase.DB0030");
		logger.entering("DB0030010Control"+":USER="+struct.getsUser_ID(),"controlINITIAL");
		//{{user_implement_dev:<controlINITIAL>
                //}}user_implement_dev:<controlINITIAL>
		logger.exiting("DB0030010Control"+":USER="+struct.getsUser_ID(),"controlINITIAL");
	...
	{
		Logger logger = Logger.getLogger("com.nec.jp.orteus.metamorBase.DB0030");
		logger.entering("DB0030010Control"+":USER="+struct.getsUser_ID(),"controlRELOAD");
		...
		try {
			...
			if (sysCostCtrlList.isEmpty()) {
				setErrorMessage("DB00007");
				setErrorParameter("SYS_COST_CTRL", "PLANT_CD", struct.getPLANT_CD());
				_mySysCostCtrl = new DB0030010Struct();
			} else {
				_mySysCostCtrl = (DB0030010Struct) sysCostCtrlList.get(0);
			}
			...
			throw new ExpjException(e);
                ...
                //}}user_implement_dev:<controlRELOAD>
		logger.exiting("DB0030010Control"+":USER="+struct.getsUser_ID(),"controlRELOAD");
	...
	{
		Logger logger = Logger.getLogger("com.nec.jp.orteus.metamorBase.DB0030");
		logger.entering("DB0030010Control"+":USER="+struct.getsUser_ID(),"controlPREPARATION");
...
//		}
		...
		struct.setList_COST_TYP_name(_costTypComboStruct.getExplanList());
                //}}user_implement_dev:<controlPREPARATION>
		logger.exiting("DB0030010Control"+":USER="+struct.getsUser_ID(),"controlPREPARATION");
...
//			e.printStackTrace();
//			throw new FoundationException("DB0030010Control",e.getMessage(),"control_eventHandllerメソッドでエラーが発生しました。");
	...
	public void createDefaultCSV(List list, String sTitle[]) throws BusinessProcessException, FoundationException {
		...
		//{{user_implement_dev:<createDefaultCSV>
			...
			e.printStackTrace();
//			objMessage.m_writer.write(Level.ALL, objMessage.getIDMsg("DB0030010-E999","CSVの出力処理"));
			throw new FoundationException("DB0030010Control",e.getMessage(),"createDefaultCSVメソッドでエラーが発生しました。");
		}
...
//				objMessage.m_writer.write(Level.ALL, objMessage.getIDMsg("DB0030010-E999","システム日付の取得処理"));
				throw new FoundationException("DB0030010Control","beginTransaction()","システム日付取得処理でエラーが発生しました。");
			}
		...
		//{{user_implement_dev:<beginTransaction>
			...
			e.printStackTrace();
//			objMessage.m_writer.write(Level.ALL, objMessage.getIDMsg("DB0030010-E999","コントロールのイベント処理"));
			throw new FoundationException("DB0030010Control",e.getMessage(),"beginTransactionメソッドでエラーが発生しました。");
		}
	 ...
	 * メンバ変数の定義
	 */
	protected DB0030010Entity entity;
	protected DB0030010Struct struct;
	...
	public void initialize() throws FoundationException
	{
		...
		entity = new DB0030010Entity();
		struct = new DB0030010Struct();
	...
	/**
	 * DB0030010クラスの標準コンストラクタ
	 */
	public DB0030010Control() throws BusinessProcessException, FoundationException
	{
		initialize();
	...
	public void setOrMsgKey(List keyList) throws MessageApiException {
		...
		for (int i = 0; msgKeyTypeArray != null && i < msgKeyTypeArray.size(); i++) {
			...
			for(int j = 0; j < keyList.size(); j++) {
				DB0030010Struct key = (DB0030010Struct)keyList.get(j);
				// メッセージキーを作成する
		...
		try {
			for (int i = 0; msgKeyTypeArray != null && i < msgKeyTypeArray.size(); i++) {
				...
				for (int j = 0; msgKeyArray != null && j < msgKeyArray.size(); j++) {
					...
					DB0030010Struct key = new DB0030010Struct();
