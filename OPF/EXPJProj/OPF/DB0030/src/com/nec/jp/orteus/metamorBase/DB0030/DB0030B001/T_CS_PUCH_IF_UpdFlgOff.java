/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/DB0030/src/com/nec/jp/orteus/metamorBase/DB0030/DB0030B001/T_CS_PUCH_IF_UpdFlgOff.java,v $
 *
...
/**
 * CLASS     : T_CS_PUCH_IF_UpdFlgOff クラス
 * ファイル・クラス説明
...
public class T_CS_PUCH_IF_UpdFlgOff extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int update() throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("T_CS_PUCH_IF_UpdFlgOff", "update", "レコード更新条件設定処理失敗");
	...
	void prepareAll() throws BatchAppException
	{
		...
		try {
			...
			BatchAppException be=new BatchAppException("T_CS_PUCH_IF_UpdFlgOff", "prepareALL(FoundationException)", "SQL文のprepareで失敗しました");
			be.addMessage(e.getErrmsg());
			throw be;
		} catch(SQLException e) {
			BatchAppException be=new BatchAppException("T_CS_PUCH_IF_UpdFlgOff", "prepareALL(SQLException)", "SQL文のprepareで失敗しました");
			throw be;
		}
	...
	/**
	 * T_CS_PUCH_IF_UpdFlgOffクラスの標準コンストラクタ
	 *
		 */
	public T_CS_PUCH_IF_UpdFlgOff(medDB0030B001 med, BatchAppConductor cdr) throws SystemException
	{
...
+"        where "
+"          CS.\"YEAR\" = to_number(?) "
+"          and CS.\"HALF_TERM_TYP\" = to_number(?) "
+"          and CS.\"COST_TYP\" = to_number(?) "
+"          and CS.\"PLANT_CD\" = ? "
+"          and CS.\"CS_PROC_CD\" = '*' "
+"        group by "
+"          CS.\"YEAR\", "
