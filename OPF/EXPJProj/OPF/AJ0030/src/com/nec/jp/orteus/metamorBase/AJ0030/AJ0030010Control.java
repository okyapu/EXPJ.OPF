/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/AJ0030/src/com/nec/jp/orteus/metamorBase/AJ0030/AJ0030010Control.java,v $
 *
...
import com.nec.jp.orteus.metamorBase.AJ0030.*;
...
//	public Object getListvalue(int x) { return this.list.get(x); }	// リストの値を返します。
	...
	public AJ0030010Struct getListvalue(int x) { return (AJ0030010Struct)(this.list.get(x)); }	// リストの値を返します。

	public AJ0030010Struct getStruct() { return this.struct; }	// Structを返します。

	public AJ0030010Struct createStruct() { return new AJ0030010Struct(); }	// 新しいStructを作って返します。

	public void setStruct(Object structname) { this.struct.setStruct((AJ0030010Struct)structname); }	// Structをセットします。
	...
	public void controlClose() throws BusinessProcessException, FoundationException
	{
		...
		logger.entering("AJ0030010Control"+":USER="+struct.getsUser_ID(),"controlClose");
			//{{user_implement_dev:<controlClose>

			// TODO: ユーザ定義のコードを記述してください

                //}}user_implement_dev:<controlClose>
		logger.exiting("AJ0030010Control"+":USER="+struct.getsUser_ID(),"controlClose");
	...
	{
		Logger logger = Logger.getLogger("com.nec.jp.orteus.metamorBase.AJ0030");
		logger.entering("AJ0030010Control"+":USER="+struct.getsUser_ID(),"controlINITIAL");
		//{{user_implement_dev:<controlINITIAL>

			// TODO: ユーザ定義のコードを記述してください

                //}}user_implement_dev:<controlINITIAL>
		logger.exiting("AJ0030010Control"+":USER="+struct.getsUser_ID(),"controlINITIAL");
	...
	{
		Logger logger = Logger.getLogger("com.nec.jp.orteus.metamorBase.AJ0030");
		logger.entering("AJ0030010Control"+":USER="+struct.getsUser_ID(),"controlRELOAD");
...
//				controlSelect();
			...
			AJ0030010Struct headerStruct = (AJ0030010Struct)tOdList.get(0);

			this.struct.setStructM(headerStruct);

			this.struct.seth_PARAMETER_OD_NO(odNo);
			...
			AJ0030010Struct tempStruct = (AJ0030010Struct)tempList.get(0);
			...
			if (headerStruct.getJOB_ODR_CD() != null
				...
				AJ0030010Struct jobOdrStruct = (AJ0030010Struct)tJobOdrList.get(0);
				...
				this.struct.setJOB_ODR_DLV_DATE(jobOdrStruct.getJOB_ODR_DLV_DATE().substring(0,10));

				int length = (jobOdrStruct.getJOB_ODR_DLV_DATE()).length();

				if(length == 16){

					this.struct.setJOB_ODR_DLV_TIME(jobOdrStruct.getJOB_ODR_DLV_DATE().substring(11,16));
			...
			this.list = entity.mV_WORK_INST_PROG_LSTCnt.read(conn, this.struct);
			long rowCount = Long.parseLong(((AJ0030010Struct)this.list.get(0)).getl_COUNT());
			if(rowCount==0){
				ExpjMessage emsg = new ExpjMessage("ZZ01106");
				msgStruct.addError(emsg);
				sysLog.warning(emsg, sysUSER_CD);
			...
			this.list = entity.mV_WORK_INST_PROG_LST.read(conn, this.struct);

			for (int i = 0; i < this.list.size(); i++) {

				AJ0030010Struct tableStruct = (AJ0030010Struct)this.list.get(i);

				tableStruct.setl_OUTSIDE_TYP_DN(this.multcombo("OUTSIDE_TYP",tableStruct.getl_OUTSIDE_TYP()));

				tableStruct.setl_ACPT_INSPC_TYP_DN(this.multcombo("ACPT_INSPC_TYP",tableStruct.getl_ACPT_INSPC_TYP()));

				//tableStruct.setl_WORK_STS_TYP_DN(this.multcombo("WORK_STS_TYP",tableStruct.getl_WORK_STS_TYP()));

				tableStruct.setl_OPR_INST_DATE(tableStruct.getl_OPR_INST_DATE());

				tableStruct.setl_OPR_INST_SLIP_ISS_DATE(tableStruct.getl_OPR_INST_SLIP_ISS_DATE());

				tableStruct.setl_OPR_INST_START_DATE(tableStruct.getl_OPR_INST_START_DATE());

				tableStruct.setl_WORK_ODR_DLV_DATE(tableStruct.getl_WORK_ODR_DLV_DATE());

				tableStruct.setl_WORK_CMPLT_DATE(tableStruct.getl_WORK_CMPLT_DATE());

				//進捗ステータスの設定

				//"遅れ":業務運用日＞製造納期かつ完了以外を満たすレコード

				String workOdrDlvDate 

						= tableStruct.getl_WORK_ODR_DLV_DATE().substring(0,10);    //製造納期

				String workStsTyp 

						= tableStruct.getl_WORK_STS_TYP();             //作業系列別作業状態区分

				

				Date dateWorkOdrDlvDate = null; //製造納期

				Date dateOprDate = null; //業務運用日

				try {

					dateWorkOdrDlvDate = Converter.strToDate(

							workOdrDlvDate,Converter.SLASH_DATE); //製造納期
				...
				if (workStsTyp == null || workStsTyp.equals("") || !workStsTyp.equals("9")) {

					//業務運用日＞製造納期

					if(dateOprDate.compareTo(dateWorkOdrDlvDate) > 0){
			...
			throw new ExpjException(e, emsg);
                ...
                //}}user_implement_dev:<controlRELOAD>
		logger.exiting("AJ0030010Control"+":USER="+struct.getsUser_ID(),"controlRELOAD");
	...
	{
		Logger logger = Logger.getLogger("com.nec.jp.orteus.metamorBase.AJ0030");
		logger.entering("AJ0030010Control"+":USER="+struct.getsUser_ID(),"controlPREPARATION");
...
//		}
		...
		//{{user_implement_dev:<controlPREPARATION>
			...
			// コンボボックス部品のデータのセットはここに記述してください。
                ...
                //}}user_implement_dev:<controlPREPARATION>
		logger.exiting("AJ0030010Control"+":USER="+struct.getsUser_ID(),"controlPREPARATION");
...
//			e.printStackTrace();
//			throw new FoundationException("AJ0030010Control",e.getMessage(),"control_eventHandllerメソッドでエラーが発生しました。");
	...
	public void createDefaultCSV(List list, String sTitle[]) throws BusinessProcessException, FoundationException {
		...
		//{{user_implement_dev:<createDefaultCSV>
			...
			e.printStackTrace();
//			objMessage.m_writer.write(Level.ALL, objMessage.getIDMsg("AJ0030010-E999","CSVの出力処理"));
			throw new FoundationException("AJ0030010Control",e.getMessage(),"createDefaultCSVメソッドでエラーが発生しました。");
		}
...
//				objMessage.m_writer.write(Level.ALL, objMessage.getIDMsg("AJ0030010-E999","システム日付の取得処理"));
				throw new FoundationException("AJ0030010Control","beginTransaction()","システム日付取得処理でエラーが発生しました。");
			}
		...
		//{{user_implement_dev:<beginTransaction>
			...
			e.printStackTrace();
//			objMessage.m_writer.write(Level.ALL, objMessage.getIDMsg("AJ0030010-E999","コントロールのイベント処理"));
			throw new FoundationException("AJ0030010Control",e.getMessage(),"beginTransactionメソッドでエラーが発生しました。");
		}
	 ...
	 * メンバ変数の定義
	 */
	protected AJ0030010Entity entity;
	protected AJ0030010Struct struct;
	...
	public void initialize() throws FoundationException
	{
		...
		entity = new AJ0030010Entity();
		struct = new AJ0030010Struct();
	...
	/**
	 * AJ0030010クラスの標準コンストラクタ
	 */
	public AJ0030010Control() throws BusinessProcessException, FoundationException
	{
		initialize();
	...
	public void setOrMsgKey(List keyList) throws MessageApiException {
		...
		for (int i = 0; msgKeyTypeArray != null && i < msgKeyTypeArray.size(); i++) {
			...
			for(int j = 0; j < keyList.size(); j++) {
				AJ0030010Struct key = (AJ0030010Struct)keyList.get(j);
				// メッセージキーを作成する
				...
				if(msgKeyType.containsKeyColumn("h_PARAMETER_OD_NO") && key.geth_PARAMETER_OD_NO() != null) {
					msgKey.setKeyValue("h_PARAMETER_OD_NO", key.geth_PARAMETER_OD_NO());
				}
				...
				if(msgKeyType.containsKeyColumn("l_PROC_EXPLOSION_FLG") && key.getl_PROC_EXPLOSION_FLG() != null) {
					msgKey.setKeyValue("l_PROC_EXPLOSION_FLG", key.getl_PROC_EXPLOSION_FLG());
				}
				if(msgKeyType.containsKeyColumn("l_WORK_STS_TYP") && key.getl_WORK_STS_TYP() != null) {
					msgKey.setKeyValue("l_WORK_STS_TYP", key.getl_WORK_STS_TYP());
				}
				if(msgKeyType.containsKeyColumn("l_WORK_STS_TYP_DN") && key.getl_WORK_STS_TYP_DN() != null) {
					msgKey.setKeyValue("l_WORK_STS_TYP_DN", key.getl_WORK_STS_TYP_DN());
				}
				...
				if(msgKeyType.containsKeyColumn("l_OPR_INST_DATE") && key.getl_OPR_INST_DATE() != null) {
					msgKey.setKeyValue("l_OPR_INST_DATE", key.getl_OPR_INST_DATE());
				}
				...
				if(msgKeyType.containsKeyColumn("l_TOTAL_DEFECT_QTY") && key.getl_TOTAL_DEFECT_QTY() != null) {
					msgKey.setKeyValue("l_TOTAL_DEFECT_QTY", key.getl_TOTAL_DEFECT_QTY());
				}
				if(msgKeyType.containsKeyColumn("l_WORK_CMPLT_DATE") && key.getl_WORK_CMPLT_DATE() != null) {
					msgKey.setKeyValue("l_WORK_CMPLT_DATE", key.getl_WORK_CMPLT_DATE());
				}
		...
		try {
			for (int i = 0; msgKeyTypeArray != null && i < msgKeyTypeArray.size(); i++) {
				...
				for (int j = 0; msgKeyArray != null && j < msgKeyArray.size(); j++) {
					...
					AJ0030010Struct key = new AJ0030010Struct();
					if(msgKeyType.containsKeyColumn("JOB_ODR_TYP_DN")) {
						key.setJOB_ODR_TYP_DN(msgKey.getKeyValue("JOB_ODR_TYP_DN"));
					}
					if(msgKeyType.containsKeyColumn("h_PARAMETER_OD_NO")) {
						key.seth_PARAMETER_OD_NO(msgKey.getKeyValue("h_PARAMETER_OD_NO"));
					}
					...
					if(msgKeyType.containsKeyColumn("l_PROC_EXPLOSION_FLG")) {
						key.setl_PROC_EXPLOSION_FLG(msgKey.getKeyValue("l_PROC_EXPLOSION_FLG"));
					}
					if(msgKeyType.containsKeyColumn("l_WORK_STS_TYP")) {
						key.setl_WORK_STS_TYP(msgKey.getKeyValue("l_WORK_STS_TYP"));
					}
					if(msgKeyType.containsKeyColumn("l_WORK_STS_TYP_DN")) {
						key.setl_WORK_STS_TYP_DN(msgKey.getKeyValue("l_WORK_STS_TYP_DN"));
					}
					...
					if(msgKeyType.containsKeyColumn("l_OPR_INST_DATE")) {
						key.setl_OPR_INST_DATE(msgKey.getKeyValue("l_OPR_INST_DATE"));
					}
					...
					if(msgKeyType.containsKeyColumn("l_TOTAL_DEFECT_QTY")) {
						key.setl_TOTAL_DEFECT_QTY(msgKey.getKeyValue("l_TOTAL_DEFECT_QTY"));
					}
					if(msgKeyType.containsKeyColumn("l_WORK_CMPLT_DATE")) {
						key.setl_WORK_CMPLT_DATE(msgKey.getKeyValue("l_WORK_CMPLT_DATE"));
					}
