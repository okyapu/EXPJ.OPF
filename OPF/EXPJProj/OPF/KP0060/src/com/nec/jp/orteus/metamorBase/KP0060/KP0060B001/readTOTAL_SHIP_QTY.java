/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/KP0060/src/com/nec/jp/orteus/metamorBase/KP0060/KP0060B001/readTOTAL_SHIP_QTY.java,v $
 *
...
/**
 * CLASS     : readTOTAL_SHIP_QTY クラス
 * ファイル・クラス説明
 * @author $Author: geng-jia $
 * @version $Revision: 1.10 $ $Date: 2015/12/03 08:54:27 $
 */

//{{user_implement_dev:header
//}}user_implement_dev:header

public class readTOTAL_SHIP_QTY extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				setIN1_R(m_med.getODR_NO());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("readTOTAL_SHIP_QTY", "read", "レコード検索条件設定処理失敗");
		...
		public String getTOTAL_SHIP_QTY_R() throws FoundationException { return m_doRs.getString(1); }
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				m_med.setTOTAL_SHIP_QTY(getTOTAL_SHIP_QTY_R());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("readTOTAL_SHIP_QTY", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("readTOTAL_SHIP_QTY", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	...
	/**
	 * readTOTAL_SHIP_QTYクラスの標準コンストラクタ
	 *
		 */
	public readTOTAL_SHIP_QTY(medKP0060B001 med, BatchAppConductor cdr) throws SystemException
	{
...
+"  NVL(SUM(TOTAL_SHIP_QTY), 0) AS TOTAL_SHIP_QTY "
+"FROM "
+"  T_SHIP_ODR "
+"WHERE "
+"  ODR_NO = ?";
