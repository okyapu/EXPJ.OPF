/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/KP0050/src/com/nec/jp/orteus/metamorBase/KP0050/KP0050B001/selectWORKDAYCOUNT.java,v $
 *
...
/**
 * CLASS     : selectWORKDAYCOUNT クラス
 * ファイル・クラス説明
 * @author $Author: geng-jia $
 * @version $Revision: 1.3 $ $Date: 2015/12/03 08:54:01 $
 */

//{{user_implement_dev:header
//}}user_implement_dev:header

public class selectWORKDAYCOUNT extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public void setCAL_DATE_START_R(String value) throws FoundationException { getAdapt_R().setString(3, value); }
		public void setCAL_DATE_END_R(String value) throws FoundationException { getAdapt_R().setString(4, value); }
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				setIN1_R(m_med.getITEM_CD());
				setIN2_R(m_med.getPLANT_CD());
				setCAL_DATE_START_R(m_med.getPLN_STR_DATE());
				setCAL_DATE_END_R(m_med.getPLN_END_DATE());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("selectWORKDAYCOUNT", "read", "レコード検索条件設定処理失敗");
		...
		public String getWORK_DAY_COUNT_R() throws FoundationException { return m_doRs.getString(1); }
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				m_med.setWORK_DAY_COUNT(getWORK_DAY_COUNT_R());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("selectWORKDAYCOUNT", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("selectWORKDAYCOUNT", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	...
	/**
	 * selectWORKDAYCOUNTクラスの標準コンストラクタ
	 *
		 */
	public selectWORKDAYCOUNT(medKP0050B001 med, BatchAppConductor cdr) throws SystemException
	{
...
+"      PLANT_CD = ? "
+"  )) "
+"  AND A.CAL_DATE >= TO_DATE(?, 'YYYY/MM/DD') "
+"  AND A.CAL_DATE <= TO_DATE(?, 'YYYY/MM/DD') "
+"  AND A.HOLIDAY_FLG = 0";
