/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/AC0150/src/com/nec/jp/orteus/metamorBase/AC0150/AC0150B001/CallSqlOdRelease_Ind.java,v $
 *
...
/**
 * CLASS     : CallSqlOdRelease_Ind クラス
 * ファイル・クラス説明
...
public class CallSqlOdRelease_Ind extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public List call() throws BatchAppException
		{
			try {
				...
				cstmt.setString(8, m_med.getpvc2SysTime());
				cstmt.setString(9, m_med.getpvc2JobOdrCd());
				cstmt.setString(10, m_med.getpvc2JobOdrDetailNo());


				cstmt.execute();

				List list = new ArrayList();
				medAC0150B001 data = null;
				...
				e.printStackTrace();
				BatchAppException be=new BatchAppException("CallSqlOdRelease_Ind", "call", "Stored Procedure条件設定処理失敗");
				...
				if (cstmt != null) {
					try {
						...
						ef.printStackTrace();
						BatchAppException bef = new BatchAppException("CallSqlOdRelease_Ind", "call", "Stored Procedureクローズ処理失敗");
		 ...
		 */
		public medAC0150B001 call(ResultSet rs) throws SQLException
		{
			if(rs == null || rs.next()==false) return null;
			medAC0150B001 data = new medAC0150B001();
			return data;
		}
	...
	/**
	 * CallSqlOdRelease_Indクラスの標準コンストラクタ
	 *
		 */
	public CallSqlOdRelease_Ind(medAC0150B001 med, BatchAppConductor cdr) throws SystemException
	{
	...
	protected static String m_calcmd = "{" + "Call SqlOdRelease_Ind(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)" + "}";
