/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/KM0060/src/com/nec/jp/orteus/metamorBase/KM0060/KM0060B001/SQLPjBudgetCostMake.java,v $
 *
...
public class SQLPjBudgetCostMake extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public List call() throws BatchAppException
		{
			try {
				...
				cstmt.setString(8, m_med.getpnumFlag());
				cstmt.setString(9, m_med.getpvc2OdrNo());
				cstmt.setString(10, m_med.getpvc2ProjectCd());
				cstmt.setString(11, m_med.getpnumEstimateNo());
				cstmt.setString(12, m_med.getpnumDetalNo());
				...
				if(rs != null) {
					...
					m_med.setpnumReturn(cstmt.getString(13));
					list.add(m_med);
				}
				...
				e.printStackTrace();
				BatchAppException be=new BatchAppException("SQLPjBudgetCostMake", "call", "Stored Procedure条件設定処理失敗");
				...
				if (cstmt != null) {
					try {
						...
						ef.printStackTrace();
						BatchAppException bef = new BatchAppException("SQLPjBudgetCostMake", "call", "Stored Procedureクローズ処理失敗");
		 ...
		 */
		public medKM0060B001 call(ResultSet rs) throws SQLException
		{
			if(rs == null || rs.next()==false) return null;
			medKM0060B001 data = new medKM0060B001();
			m_med.setpnumReturn(cstmt.getString(13));
			return data;
		}
	 ...
	 *
		 */
	public SQLPjBudgetCostMake(medKM0060B001 med, BatchAppConductor cdr) throws SystemException
	{
	...
	protected static String m_calcmd = "{" + "call SQLPjBudgetCostMake(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)" + "}";
