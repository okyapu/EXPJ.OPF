/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/KM0060/src/com/nec/jp/orteus/metamorBase/KM0060/KM0060B001/T_PROJECT_cnt.java,v $
 *
...
public class T_PROJECT_cnt extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("T_PROJECT_cnt", "read", "レコード検索条件設定処理失敗");
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				m_med.setPJ_count(getCOUNT_R());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("T_PROJECT_cnt", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("T_PROJECT_cnt", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	...
	/**
	 * T_PROJECT_cntクラスの標準コンストラクタ
	 *
		 */
	public T_PROJECT_cnt(medKM0060B001 med, BatchAppConductor cdr) throws SystemException
	{
