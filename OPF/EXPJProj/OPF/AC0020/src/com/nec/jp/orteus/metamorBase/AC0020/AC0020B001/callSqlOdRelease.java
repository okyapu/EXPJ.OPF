/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/AC0020/src/com/nec/jp/orteus/metamorBase/AC0020/AC0020B001/callSqlOdRelease.java,v $
 *
...
public class callSqlOdRelease extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public List call() throws BatchAppException
		{
			try {
				...
				cstmt.setString(8, m_med.getpvc2ManagementTyp());
				cstmt.setString(9, m_med.getpvc2OutsideTyp());
				cstmt.setString(10, m_med.getpvc2JobOdrCd());
				cstmt.setString(11, m_med.getpvc2JobOdrDetailNo());
				cstmt.setString(12, m_med.getpvc2Entry1Typ());
				cstmt.setString(13, m_med.getpvc2Entry2Typ());
				cstmt.setString(14, m_med.getpvc2EntryDate());


				cstmt.execute();

				List list = new ArrayList();
				medAC0020B001 data = null;
				...
				e.printStackTrace();
				BatchAppException be=new BatchAppException("callSqlOdRelease", "call", "Stored Procedure条件設定処理失敗");
				...
				if (cstmt != null) {
					try {
						...
						ef.printStackTrace();
						BatchAppException bef = new BatchAppException("callSqlOdRelease", "call", "Stored Procedureクローズ処理失敗");
		 ...
		 */
		public medAC0020B001 call(ResultSet rs) throws SQLException
		{
			if(rs == null || rs.next()==false) return null;
			medAC0020B001 data = new medAC0020B001();
			return data;
		}
	...
	/**
	 * callSqlOdReleaseクラスの標準コンストラクタ
	 *
		 */
	public callSqlOdRelease(medAC0020B001 med, BatchAppConductor cdr) throws SystemException
	{
	...
	protected static String m_calcmd = "{" + "call SqlOdRelease(?,?,?,?,?,?,?,?,?,?,?,?,?,?)" + "}";
