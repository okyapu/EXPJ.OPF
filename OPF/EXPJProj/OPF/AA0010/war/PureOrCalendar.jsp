...
<%		for (int i = 0; i < 7; i++){
            ...
            // org.apache.struts.taglib.TagUtilsのソースを参考します
			class TagFilter {
		...
		TagFilter filter = new TagFilter();
			...
			sYYYYMMDD = filter.filter(sYYYYMMDD);
...
<%			} else {	%>
      <A HREF="javascript:function v() {
        var dateFormat = document.getElementsByName('<%= formatKey %>')[0].value;          
		if(self.opener.currentRow == 'false') {
			opener.document.getElementsByName(self.opener.formName)[0][self.opener.targetName].value=(dateFormat);
		} else {
			opener.document.getElementsByName(self.opener.formName)[0][self.opener.targetName][self.opener.currentRow].value=(dateFormat);
		};
		self.close();
	  };
	  v()">
