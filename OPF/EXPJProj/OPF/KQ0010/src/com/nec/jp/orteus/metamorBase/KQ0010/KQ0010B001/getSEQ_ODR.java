/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/KQ0010/src/com/nec/jp/orteus/metamorBase/KQ0010/KQ0010B001/getSEQ_ODR.java,v $
 *
...
public class getSEQ_ODR extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("getSEQ_ODR", "read", "レコード検索条件設定処理失敗");
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				m_med.setstrODR_NO(getODR_NO_R());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("getSEQ_ODR", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("getSEQ_ODR", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	...
	/**
	 * getSEQ_ODRクラスの標準コンストラクタ
	 *
		 */
	public getSEQ_ODR(medKQ0010B001 med, BatchAppConductor cdr) throws SystemException
	{
