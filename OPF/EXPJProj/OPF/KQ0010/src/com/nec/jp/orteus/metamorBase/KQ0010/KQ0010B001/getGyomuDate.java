/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/KQ0010/src/com/nec/jp/orteus/metamorBase/KQ0010/KQ0010B001/getGyomuDate.java,v $
 *
...
public class getGyomuDate extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				setPLANT_CD_R(m_med.getstrPLANT_CD());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("getGyomuDate", "read", "レコード検索条件設定処理失敗");
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				m_med.setstrBUSINESS_OPR_DATE(getBUSINESS_OPR_DATE_R());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("getGyomuDate", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("getGyomuDate", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	 ...
	 *
		 */
	public getGyomuDate(medKQ0010B001 med, BatchAppConductor cdr) throws SystemException
	{
