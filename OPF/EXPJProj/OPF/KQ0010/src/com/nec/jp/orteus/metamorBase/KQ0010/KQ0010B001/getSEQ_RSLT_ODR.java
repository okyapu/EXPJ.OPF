/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/KQ0010/src/com/nec/jp/orteus/metamorBase/KQ0010/KQ0010B001/getSEQ_RSLT_ODR.java,v $
 *
...
/**
 * CLASS     : getSEQ_RSLT_ODR クラス
 * ファイル・クラス説明
 * @author $Author: geng-jia $
 * @version $Revision: 1.21 $ $Date: 2015/12/03 08:12:36 $
 */
...
public class getSEQ_RSLT_ODR extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("getSEQ_RSLT_ODR", "read", "レコード検索条件設定処理失敗");
		...
		public boolean wasNull_RSLT_CTL_SEQ_CD_R() throws FoundationException { return m_doRs.wasNull(1); }
		public Number getRSLT_CTL_SEQ_CD_R() throws FoundationException { return m_doRs.getDouble(1); }
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				if(wasNull_RSLT_CTL_SEQ_CD_R()==true) {
					m_med.setstrRSLT_CTL_SEQ_CD(null);
				} else {
					m_med.setstrRSLT_CTL_SEQ_CD(new Double(getRSLT_CTL_SEQ_CD_R().doubleValue()));
				}
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("getSEQ_RSLT_ODR", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("getSEQ_RSLT_ODR", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	 ...
	 *
		 */
	public getSEQ_RSLT_ODR(medKQ0010B001 med, BatchAppConductor cdr) throws SystemException
	{
