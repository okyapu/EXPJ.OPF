/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/AD0080/src/com/nec/jp/orteus/metamorBase/AD0080/AD0080B001/lockT_OPR_RSLT_TEMP.java,v $
 *
...
public class lockT_OPR_RSLT_TEMP extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				setIN1_R(m_med.getOPR_RSLT_TEMP_CTL_NO());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("lockT_OPR_RSLT_TEMP", "read", "レコード検索条件設定処理失敗");
		...
		public String getOPR_RSLT_TEMP_CTL_NO_R() throws FoundationException { return m_doRs.getString(1); }
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("lockT_OPR_RSLT_TEMP", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("lockT_OPR_RSLT_TEMP", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	...
	/**
	 * lockT_OPR_RSLT_TEMPクラスの標準コンストラクタ
	 *
		 */
	public lockT_OPR_RSLT_TEMP(medAD0080B001 med, BatchAppConductor cdr) throws SystemException
	{
