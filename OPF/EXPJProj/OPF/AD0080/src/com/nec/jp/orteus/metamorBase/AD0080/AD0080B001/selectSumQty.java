/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/AD0080/src/com/nec/jp/orteus/metamorBase/AD0080/AD0080B001/selectSumQty.java,v $
 *
...
/**
 * CLASS     : selectSumQty クラス
 * ファイル・クラス説明
 * @author $Author: geng-jia $
 * @version $Revision: 1.12 $ $Date: 2015/12/22 09:52:55 $
 */

//{{user_implement_dev:header
//}}user_implement_dev:header

public class selectSumQty extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				setIN2_R(m_med.getOPR_INST_CD());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("selectSumQty", "read", "レコード検索条件設定処理失敗");
		...
		public String getSUM_ACPT_QTY_R() throws FoundationException { return m_doRs.getString(1); }
		public String getSUM_DEFECT_QTY_R() throws FoundationException { return m_doRs.getString(2); }
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				m_med.setSUM_ACPT_QTY(getSUM_ACPT_QTY_R());
				m_med.setSUM_DEFECT_QTY(getSUM_DEFECT_QTY_R());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("selectSumQty", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("selectSumQty", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	...
	/**
	 * selectSumQtyクラスの標準コンストラクタ
	 *
		 */
	public selectSumQty(medAD0080B001 med, BatchAppConductor cdr) throws SystemException
	{
...
+"  NVL(TO_CHAR(SUM(ACPT_QTY), 'fm999999999999999990.0999'), '0.0') AS SUM_ACPT_QTY, "
+"  NVL(TO_CHAR(SUM(DEFECT_QTY), 'fm999999999999999990.0999'), '0.0') AS SUM_DEFECT_QTY "
+"from "
+"  T_OPR_RSLT "
+"where "
+"  OPR_INST_CD = ?";
