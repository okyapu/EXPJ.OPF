/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/KU0070/src/com/nec/jp/orteus/metamorBase/KU0070/KU0070010Control.java,v $
 *
...
import com.nec.jp.orteus.metamorBase.KU0070.*;
...
//	public Object getListvalue(int x) { return this.list.get(x); }	// リストの値を返します。
	public KU0070010Struct getListvalue(int x) { return (KU0070010Struct)(this.list.get(x)); }	// リストの値を返します。
	public KU0070010Struct getStruct() { return this.struct; }	// Structを返します。
	public KU0070010Struct createStruct() { return new KU0070010Struct(); }	// 新しいStructを作って返します。
	public void setStruct(Object structname) { this.struct.setStruct((KU0070010Struct)structname); }	// Structをセットします。
	...
	private boolean getMaxLine(long rowCount) throws FoundationException {
		try {
			...
			if (maxLine != 0 && rowCount == maxLine + 1) {
				...
				emsg = new ExpjMessage("ZZ01006", "DLV_LOC_CD:"+ struct.getIN_DLV_LOC_CD());
				sysLog.warning(emsg, getsysUSER_CD()); // エラー詳細
				emsg = new ExpjMessage("ZZ01006", "CHRG_PSN_CD:"+ struct.getIN_CHRG_PSN_CD());
				sysLog.warning(emsg, getsysUSER_CD()); // エラー詳細
				emsg = new ExpjMessage("ZZ01006", "FROM_SCDL_SHIP_DATE:"+ struct.getFROM_SCDL_SHIP_DATE());
				sysLog.warning(emsg, getsysUSER_CD()); // エラー詳細
				emsg = new ExpjMessage("ZZ01006", "TO_SCDL_SHIP_DATE:"+ struct.getTO_SCDL_SHIP_DATE());
				sysLog.warning(emsg, getsysUSER_CD()); // エラー詳細
	...
	{
		Logger logger = Logger.getLogger("com.nec.jp.orteus.metamorBase.KU0070");
		logger.entering("KU0070010Control"+":USER="+struct.getsUser_ID(),"controlSelect");
		...
		try {
			...
			long rowCount = Long.parseLong(((KU0070010Struct)list.get(0)).getl_COUNT());
			if( rowCount == 0 ){
			    //データが存在しない
			...
			KU0070010Struct TempStruct;
            for (int i = 0; i < this.list.size(); i++) {
                 TempStruct = (KU0070010Struct)list.get(i);
	    	...
	    	sysLog.warning(emsg, sysUSER_CD);
                ...
                //}}user_implement_dev:<controlSelect>
		logger.exiting("KU0070010Control"+":USER="+struct.getsUser_ID(),"controlSelect");
	...
	{
		Logger logger = Logger.getLogger("com.nec.jp.orteus.metamorBase.KU0070");
		logger.entering("KU0070010Control"+":USER="+struct.getsUser_ID(),"controlClose");
			//{{user_implement_dev:<controlClose>
			// TODO: ユーザ定義のコードを記述してください
                //}}user_implement_dev:<controlClose>
		logger.exiting("KU0070010Control"+":USER="+struct.getsUser_ID(),"controlClose");
	...
	{
		Logger logger = Logger.getLogger("com.nec.jp.orteus.metamorBase.KU0070");
		logger.entering("KU0070010Control"+":USER="+struct.getsUser_ID(),"controlClear");
			...
			// TODO: ユーザ定義のコードを記述してください
                ...
                //}}user_implement_dev:<controlClear>
		logger.exiting("KU0070010Control"+":USER="+struct.getsUser_ID(),"controlClear");

		return;
	}

	/**
	 * 選択情報登録ボタン押下時に実行される処理です。
	 *
	 */
	public void controlGoSub() throws BusinessProcessException, FoundationException
	{
		Logger logger = Logger.getLogger("com.nec.jp.orteus.metamorBase.KU0070");
		logger.entering("KU0070010Control"+":USER="+struct.getsUser_ID(),"controlGoSub");
			//{{user_implement_dev:<controlGoSub>
			// TODO: ユーザ定義のコードを記述してください
		try{
			//一覧にて１行以上を選択、かつ選択したすべてのデータの「得意先コード」、「納品場所コード」、「出荷予定日」の値が同じではないの場合
			String oldCustCd = "";
			String oldDlvLocCd = "";
			String oldDesinatedShipDate = "";
			String oldShipOdrNo = "";
			KU0070010Struct structtemp = new KU0070010Struct();
			struct.init();
			for (int m = 0; m < list.size(); m++){
		    	structtemp = (KU0070010Struct)list.get(m);
		    	structtemp.setSELECT_MARK("0");
		    }
			for( int i = 0; i < struct.getList_CUST_CD().size(); i++){
			    if((!"".equals(oldCustCd)&& !oldCustCd.equals(struct.getList_CUST_CD().get(i)))
			    	||!"".equals(oldDlvLocCd) && !oldDlvLocCd.equals(struct.getList_DLV_LOC_CD().get(i))
			    	||!"".equals(oldDesinatedShipDate) && !oldDesinatedShipDate.equals(struct.getList_SHIP_DATE().get(i))){
			    	struct.setw_Flag("0");
			    	//エラーメッセージ出力
			    	ExpjMessage emsg = new ExpjMessage("KU00112");
			        msgStruct.addError(emsg);	// エラーメッセージとして登録
			        sysLog.warning(emsg, getsysUSER_CD()); //エラー内容
			        return;
			    }
			    oldCustCd = (String)struct.getList_CUST_CD().get(i);
			    oldDlvLocCd = (String)struct.getList_DLV_LOC_CD().get(i);
			    oldDesinatedShipDate = (String)struct.getList_SHIP_DATE().get(i);
			    oldShipOdrNo = (String)struct.getList_SHIP_ODR_NO().get(i);
			    for (int j = 0; j < list.size(); j++){
			    	structtemp = (KU0070010Struct)list.get(j);
			    	if (oldShipOdrNo.equals(structtemp.getSHIP_ODR_NO())){
				        structtemp.setSELECT_MARK("1");
			    	}
			    }
			}
			struct.setw_Flag("1");
	    	...
	    	sysLog.warning(emsg, sysUSER_CD);
                ...
                //}}user_implement_dev:<controlGoSub>
		logger.exiting("KU0070010Control"+":USER="+struct.getsUser_ID(),"controlGoSub");
	...
	{
		Logger logger = Logger.getLogger("com.nec.jp.orteus.metamorBase.KU0070");
		logger.entering("KU0070010Control"+":USER="+struct.getsUser_ID(),"controlINITIAL");
		//{{user_implement_dev:<controlINITIAL>
			// TODO: ユーザ定義のコードを記述してください
                //}}user_implement_dev:<controlINITIAL>
		logger.exiting("KU0070010Control"+":USER="+struct.getsUser_ID(),"controlINITIAL");
	...
	{
		Logger logger = Logger.getLogger("com.nec.jp.orteus.metamorBase.KU0070");
		logger.entering("KU0070010Control"+":USER="+struct.getsUser_ID(),"controlRELOAD");
		...
		try{
			...
			if(listKaisyaCd == null || listKaisyaCd.size() != 1){
				...
				ExpjMessage emsg = new ExpjMessage("KQ00039");
				 ...
				 struct.setstrCOMPANY_CD(((KU0070010Struct)listKaisyaCd.get(0)).getstrCOMPANY_CD());
			 }
			 listKaisyaCd = null;
			...
			throw ee;
                ...
                //}}user_implement_dev:<controlRELOAD>
		logger.exiting("KU0070010Control"+":USER="+struct.getsUser_ID(),"controlRELOAD");
	...
	{
		Logger logger = Logger.getLogger("com.nec.jp.orteus.metamorBase.KU0070");
		logger.entering("KU0070010Control"+":USER="+struct.getsUser_ID(),"controlPREPARATION");
...
//		}
		...
		//{{user_implement_dev:<controlPREPARATION>
			...
			struct.setw_Flag("0");
                //}}user_implement_dev:<controlPREPARATION>
		logger.exiting("KU0070010Control"+":USER="+struct.getsUser_ID(),"controlPREPARATION");
...
//			e.printStackTrace();
//			throw new FoundationException("KU0070010Control",e.getMessage(),"control_eventHandllerメソッドでエラーが発生しました。");
	...
	public void createDefaultCSV(List list, String sTitle[]) throws BusinessProcessException, FoundationException {
		...
		//{{user_implement_dev:<createDefaultCSV>
			...
			e.printStackTrace();
//			objMessage.m_writer.write(Level.ALL, objMessage.getIDMsg("KU0070010-E999","CSVの出力処理"));
			throw new FoundationException("KU0070010Control",e.getMessage(),"createDefaultCSVメソッドでエラーが発生しました。");
		}
...
//				objMessage.m_writer.write(Level.ALL, objMessage.getIDMsg("KU0070010-E999","システム日付の取得処理"));
				throw new FoundationException("KU0070010Control","beginTransaction()","システム日付取得処理でエラーが発生しました。");
			}
		...
		//{{user_implement_dev:<beginTransaction>
			...
			e.printStackTrace();
//			objMessage.m_writer.write(Level.ALL, objMessage.getIDMsg("KU0070010-E999","コントロールのイベント処理"));
			throw new FoundationException("KU0070010Control",e.getMessage(),"beginTransactionメソッドでエラーが発生しました。");
		}
	 ...
	 * メンバ変数の定義
	 */
	protected KU0070010Entity entity;
	protected KU0070010Struct struct;
	...
	public void initialize() throws FoundationException
	{
		...
		entity = new KU0070010Entity();
		struct = new KU0070010Struct();
	 ...
	 */
	public KU0070010Control() throws BusinessProcessException, FoundationException
	{
		initialize();
	...
	public void setOrMsgKey(List keyList) throws MessageApiException {
		...
		for (int i = 0; msgKeyTypeArray != null && i < msgKeyTypeArray.size(); i++) {
			...
			for(int j = 0; j < keyList.size(); j++) {
				KU0070010Struct key = (KU0070010Struct)keyList.get(j);
				// メッセージキーを作成する
				MessageKey msgKey = new MessageKey(tableName, keyColumnName);
				if(msgKeyType.containsKeyColumn("SELECT_MARK") && key.getSELECT_MARK() != null) {
					msgKey.setKeyValue("SELECT_MARK", key.getSELECT_MARK());
				}
				if(msgKeyType.containsKeyColumn("w_Flag") && key.getw_Flag() != null) {
					msgKey.setKeyValue("w_Flag", key.getw_Flag());
				}
				...
				if(msgKeyType.containsKeyColumn("ACTUAL_UNIT_PRICE") && key.getACTUAL_UNIT_PRICE() != null) {
					msgKey.setKeyValue("ACTUAL_UNIT_PRICE", key.getACTUAL_UNIT_PRICE());
				}
				...
				if(msgKeyType.containsKeyColumn("IN_CHRG_PSN_CD") && key.getIN_CHRG_PSN_CD() != null) {
					msgKey.setKeyValue("IN_CHRG_PSN_CD", key.getIN_CHRG_PSN_CD());
				}
				if(msgKeyType.containsKeyColumn("FROM_SCDL_SHIP_DATE") && key.getFROM_SCDL_SHIP_DATE() != null) {
					msgKey.setKeyValue("FROM_SCDL_SHIP_DATE", key.getFROM_SCDL_SHIP_DATE());
				}
				if(msgKeyType.containsKeyColumn("TO_SCDL_SHIP_DATE") && key.getTO_SCDL_SHIP_DATE() != null) {
					msgKey.setKeyValue("TO_SCDL_SHIP_DATE", key.getTO_SCDL_SHIP_DATE());
				}
		...
		try {
			for (int i = 0; msgKeyTypeArray != null && i < msgKeyTypeArray.size(); i++) {
				...
				for (int j = 0; msgKeyArray != null && j < msgKeyArray.size(); j++) {
					...
					KU0070010Struct key = new KU0070010Struct();
					if(msgKeyType.containsKeyColumn("SELECT_MARK")) {
						key.setSELECT_MARK(msgKey.getKeyValue("SELECT_MARK"));
					}
					if(msgKeyType.containsKeyColumn("w_Flag")) {
						key.setw_Flag(msgKey.getKeyValue("w_Flag"));
					}
					...
					if(msgKeyType.containsKeyColumn("ACTUAL_UNIT_PRICE")) {
						key.setACTUAL_UNIT_PRICE(msgKey.getKeyValue("ACTUAL_UNIT_PRICE"));
					}
					...
					if(msgKeyType.containsKeyColumn("IN_CHRG_PSN_CD")) {
						key.setIN_CHRG_PSN_CD(msgKey.getKeyValue("IN_CHRG_PSN_CD"));
					}
					if(msgKeyType.containsKeyColumn("FROM_SCDL_SHIP_DATE")) {
						key.setFROM_SCDL_SHIP_DATE(msgKey.getKeyValue("FROM_SCDL_SHIP_DATE"));
					}
					if(msgKeyType.containsKeyColumn("TO_SCDL_SHIP_DATE")) {
						key.setTO_SCDL_SHIP_DATE(msgKey.getKeyValue("TO_SCDL_SHIP_DATE"));
					}
