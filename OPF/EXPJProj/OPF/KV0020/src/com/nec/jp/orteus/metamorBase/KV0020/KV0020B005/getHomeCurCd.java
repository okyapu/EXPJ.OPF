/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/KV0020/src/com/nec/jp/orteus/metamorBase/KV0020/KV0020B005/getHomeCurCd.java,v $
 *
...
public class getHomeCurCd extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("getHomeCurCd", "read", "レコード検索条件設定処理失敗");
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				m_med.setCUR_CD(getHOME_CUR_CD_R());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("getHomeCurCd", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("getHomeCurCd", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	...
	/**
	 * getHomeCurCdクラスの標準コンストラクタ
	 *
		 */
	public getHomeCurCd(medKV0020B005 med, BatchAppConductor cdr) throws SystemException
	{
