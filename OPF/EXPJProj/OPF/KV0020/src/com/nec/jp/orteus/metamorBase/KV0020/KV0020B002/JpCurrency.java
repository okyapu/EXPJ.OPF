/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/KV0020/src/com/nec/jp/orteus/metamorBase/KV0020/KV0020B002/JpCurrency.java,v $
 *
...
public class JpCurrency extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("JpCurrency", "read", "レコード検索条件設定処理失敗");
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				m_med.setstrJP_CUR(getHOME_CUR_CD_R());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("JpCurrency", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("JpCurrency", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	...
	/**
	 * JpCurrencyクラスの標準コンストラクタ
	 *
		 */
	public JpCurrency(medKV0020B002 med, BatchAppConductor cdr) throws SystemException
	{
