/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/KV0020/src/com/nec/jp/orteus/metamorBase/KV0020/KV0020B005/chkSysInstallOptions.java,v $
 *
...
/**
 * CLASS     : chkSysInstallOptions クラス
 * ファイル・クラス説明
...
public class chkSysInstallOptions extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("chkSysInstallOptions", "read", "レコード検索条件設定処理失敗");
		...
		public String getAR_INSTALL_CNT_R() throws FoundationException { return m_doRs.getString(1); }
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				m_med.setAR_INSTALL_CNT(getAR_INSTALL_CNT_R());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("chkSysInstallOptions", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("chkSysInstallOptions", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	 ...
	 *
		 */
	public chkSysInstallOptions(medKV0020B005 med, BatchAppConductor cdr) throws SystemException
	{
	...
	// 各状態で実行するsql文の定義
	protected static String m_selcmd = "select count(*) from SYS_INSTALL_OPTIONS  "
