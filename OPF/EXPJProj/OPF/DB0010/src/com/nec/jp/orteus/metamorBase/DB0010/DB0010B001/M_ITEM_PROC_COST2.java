/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/DB0010/src/com/nec/jp/orteus/metamorBase/DB0010/DB0010B001/M_ITEM_PROC_COST2.java,v $
 *
...
public class M_ITEM_PROC_COST2 extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public void setIN5_R(String value) throws FoundationException { getAdapt_R().setString(3, value); }
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				setIN5_R(m_med.getPROC_CD());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("M_ITEM_PROC_COST2", "read", "レコード検索条件設定処理失敗");
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("M_ITEM_PROC_COST2", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("M_ITEM_PROC_COST2", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	...
	/**
	 * M_ITEM_PROC_COST2クラスの標準コンストラクタ
	 *
		 */
	public M_ITEM_PROC_COST2(medDB0010B001 med, BatchAppConductor cdr) throws SystemException
	{
...
+"  M_ITEM_PROC_COST.\"CS_PROC_CD\" as \"CS_PROC_CD\" "
+"from "
+"  M_ITEM_PROC_COST "
+"where "
