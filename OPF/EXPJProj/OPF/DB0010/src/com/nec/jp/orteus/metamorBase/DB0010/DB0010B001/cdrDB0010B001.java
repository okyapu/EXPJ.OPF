/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/DB0010/src/com/nec/jp/orteus/metamorBase/DB0010/DB0010B001/cdrDB0010B001.java,v $
 *
...
/**
 * CLASS     : cdrDB0010B001 クラス
 * ファイル・クラス説明
 * @author $Author: xl-hs-zzw $
 * @version $Revision: 1.7 $ $Date: 2014/11/05 07:20:43 $
 */

//{{user_implement_dev:header
//}}user_implement_dev:header

public class cdrDB0010B001 extends Object
		implements IBatchAppCommon
{
	...
	public void initialize() throws BatchAppException, SystemException
	{
		try {
			...
			m_med.setM_PROC_GRP( new M_PROC_GRP(m_med, m_conductor) );
			m_med.setT_ITEM_CS_PROC_IF( new T_ITEM_CS_PROC_IF(m_med, m_conductor) );
			m_med.setT_ITEM_CS_PROC_IF_delete( new T_ITEM_CS_PROC_IF_delete(m_med, m_conductor) );
			...
			m_med.setM_CS_PROC( new M_CS_PROC(m_med, m_conductor) );
			m_med.setM_ITEM_PROC_COST( new M_ITEM_PROC_COST(m_med, m_conductor) );
			m_med.setT_ITEM_CS_PROC_IF_updated( new T_ITEM_CS_PROC_IF_updated(m_med, m_conductor) );
			m_med.setM_CS_PROC_delete( new M_CS_PROC_delete(m_med, m_conductor) );
			m_med.setM_ITEM_PROC_COST2( new M_ITEM_PROC_COST2(m_med, m_conductor) );
			m_med.setT_CS_ERROR( new T_CS_ERROR(m_med, m_conductor) );
			m_med.setM_CONS_UNIT_COST( new M_CONS_UNIT_COST(m_med, m_conductor) );
			m_med.setSEQ_CS_ERROR_CD( new SEQ_CS_ERROR_CD(m_med, m_conductor) );
			m_med.setdeleteM_CS_PROC( new deleteM_CS_PROC(m_med, m_conductor) );
			m_med.setCtrlMain( new CtrlMain(m_med, m_conductor) );
			m_conductor.addObject( m_med.getM_PROC_GRP() );
			m_conductor.addObject( m_med.getT_ITEM_CS_PROC_IF() );
			m_conductor.addObject( m_med.getT_ITEM_CS_PROC_IF_delete() );
			...
			m_conductor.addObject( m_med.getM_CS_PROC() );
			m_conductor.addObject( m_med.getM_ITEM_PROC_COST() );
			m_conductor.addObject( m_med.getT_ITEM_CS_PROC_IF_updated() );
			m_conductor.addObject( m_med.getM_CS_PROC_delete() );
			m_conductor.addObject( m_med.getM_ITEM_PROC_COST2() );
			m_conductor.addObject( m_med.getT_CS_ERROR() );
			m_conductor.addObject( m_med.getM_CONS_UNIT_COST() );
			m_conductor.addObject( m_med.getSEQ_CS_ERROR_CD() );
			m_conductor.addObject( m_med.getdeleteM_CS_PROC() );
	...
	public int getType() { return m_conductor.getType(); }
        ...
        public int beginProc() throws BatchAppException
        {
         try {
          ...
          setCommitFlag(false);
          BatchAppException be=new BatchAppException("cdrDB0010B001", "beginProc", "conductorの初期処理に失敗しました");
	...
	medDB0010B001 m_med=new medDB0010B001();

	// BatchAppConductorクラスインスタンス
	BatchAppConductor m_conductor=new BatchAppConductor("DB0010B001");
	...
	/**
	 * ctrDB0010B001クラス 標準コンストラクタ
	 *
		 */
	public cdrDB0010B001() throws BatchAppException
	{
		initialize();
