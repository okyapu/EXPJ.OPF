/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/DB0010/src/com/nec/jp/orteus/metamorBase/DB0010/DB0010B001/M_CONS_UNIT_COST.java,v $
 *
...
public class M_CONS_UNIT_COST extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				setIN3_R(m_med.getITEM_CD());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("M_CONS_UNIT_COST", "read", "レコード検索条件設定処理失敗");
		...
		public String getONEROUS_FLG_R() throws FoundationException { return m_doRs.getString(1); }
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				m_med.setONEROUS_CONS_FLG(getONEROUS_FLG_R());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("M_CONS_UNIT_COST", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("M_CONS_UNIT_COST", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	 ...
	 *
		 */
	public M_CONS_UNIT_COST(medDB0010B001 med, BatchAppConductor cdr) throws SystemException
	{
...
+"  to_char(nvl(min(ONEROUS_FLG), 0)) AS \"ONEROUS_FLG\" "
+"from "
+"  M_CONS_UNIT_COST "
+"where "
