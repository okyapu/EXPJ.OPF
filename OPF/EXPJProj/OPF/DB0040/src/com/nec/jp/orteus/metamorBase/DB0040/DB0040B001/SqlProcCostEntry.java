/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/DB0040/src/com/nec/jp/orteus/metamorBase/DB0040/DB0040B001/SqlProcCostEntry.java,v $
 *
...
public class SqlProcCostEntry extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public List call() throws BatchAppException
		{
			try {
				...
				cstmt.setString(8, m_med.getpvc2TargetPlantCd());
				cstmt.setString(9, m_med.getpvc2Year());
				cstmt.setString(10, m_med.getpvc2HalfTermTyp());
				cstmt.setString(11, m_med.getpvc2CostTyp());
				cstmt.setString(12, m_med.getpvc2BatchTyp());


				cstmt.execute();

				List list = new ArrayList();
				medDB0040B001 data = null;
				...
				e.printStackTrace();
				BatchAppException be=new BatchAppException("SqlProcCostEntry", "call", "Stored Procedure条件設定処理失敗");
				...
				if (cstmt != null) {
					try {
						...
						ef.printStackTrace();
						BatchAppException bef = new BatchAppException("SqlProcCostEntry", "call", "Stored Procedureクローズ処理失敗");
		 ...
		 */
		public medDB0040B001 call(ResultSet rs) throws SQLException
		{
			if(rs == null || rs.next()==false) return null;
			medDB0040B001 data = new medDB0040B001();
			return data;
		}
	...
	/**
	 * SqlProcCostEntryクラスの標準コンストラクタ
	 *
		 */
	public SqlProcCostEntry(medDB0040B001 med, BatchAppConductor cdr) throws SystemException
	{
	...
	protected static String m_calcmd = "{" + "call SqlProcCostEntry(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)" + "}";
