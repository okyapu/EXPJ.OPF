/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/KV0030/src/com/nec/jp/orteus/metamorBase/KV0030/KV0030B001/readSYS_MY_COMPANY.java,v $
 *
...
public class readSYS_MY_COMPANY extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("readSYS_MY_COMPANY", "read", "レコード検索条件設定処理失敗");
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("readSYS_MY_COMPANY", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("readSYS_MY_COMPANY", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	...
	/**
	 * readSYS_MY_COMPANYクラスの標準コンストラクタ
	 *
		 */
	public readSYS_MY_COMPANY(medKV0030B001 med, BatchAppConductor cdr) throws SystemException
	{
	...
	// 各状態で実行するsql文の定義
	protected static String m_selcmd = "select SYS_MY_COMPANY.COMPANY_CD AS COMPANY_CD "
+"  from SYS_MY_COMPANY "
