/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/KV0030/src/com/nec/jp/orteus/metamorBase/KV0030/KV0030B001/readM_PLANT.java,v $
 *
...
public class readM_PLANT extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public void setinPLANT_CD_R(String value) throws FoundationException { getAdapt_R().setString(1, value); }
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				setinPLANT_CD_R(m_med.getinPLANT_CD());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("readM_PLANT", "read", "レコード検索条件設定処理失敗");
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("readM_PLANT", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("readM_PLANT", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	 ...
	 *
		 */
	public readM_PLANT(medKV0030B001 med, BatchAppConductor cdr) throws SystemException
	{
	...
	// 各状態で実行するsql文の定義
	protected static String m_selcmd = "select M_PLANT.PLANT_CD as PLANT_CD "
+"  from M_PLANT "
+" where M_PLANT.PLANT_CD like ?";
