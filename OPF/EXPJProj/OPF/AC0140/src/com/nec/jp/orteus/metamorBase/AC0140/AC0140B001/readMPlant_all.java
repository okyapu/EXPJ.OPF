/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/AC0140/src/com/nec/jp/orteus/metamorBase/AC0140/AC0140B001/readMPlant_all.java,v $
 *
...
public class readMPlant_all extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("readMPlant_all", "read", "レコード検索条件設定処理失敗");
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				m_med.setcurrentPlantCd(getPLANT_CD_R());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("readMPlant_all", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("readMPlant_all", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	...
	/**
	 * readMPlant_allクラスの標準コンストラクタ
	 *
		 */
	public readMPlant_all(medAC0140B001 med, BatchAppConductor cdr) throws SystemException
	{
