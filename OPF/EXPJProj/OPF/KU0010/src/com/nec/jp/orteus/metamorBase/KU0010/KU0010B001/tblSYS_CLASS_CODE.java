/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/KU0010/src/com/nec/jp/orteus/metamorBase/KU0010/KU0010B001/tblSYS_CLASS_CODE.java,v $
 *
...
public class tblSYS_CLASS_CODE extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("tblSYS_CLASS_CODE", "read", "レコード検索条件設定処理失敗");
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				m_med.setstrSYS_CLASS_CODE(getCODE_R());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("tblSYS_CLASS_CODE", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("tblSYS_CLASS_CODE", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	...
	/**
	 * tblSYS_CLASS_CODEクラスの標準コンストラクタ
	 *
		 */
	public tblSYS_CLASS_CODE(medKU0010B001 med, BatchAppConductor cdr) throws SystemException
	{
...
+"  and SYS_CLASS_CODE.\"CLASS_CODE\" = 'PLANT_BATCH'";
