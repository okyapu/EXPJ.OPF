/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/KU0010/src/com/nec/jp/orteus/metamorBase/KU0010/KU0010B001/leedTime.java,v $
 *
 ...
 * @version $Revision: 1.18 $ $Date: 2015/12/03 08:16:02 $
 */

//{{user_implement_dev:header
//}}user_implement_dev:header

public class leedTime extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("leedTime", "read", "レコード検索条件設定処理失敗");
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				m_med.setleedTimeSYS_CLASS(getSYS_CLASS_R());
				m_med.setleedTimeCLASS_CODE(getCLASS_CODE_R());
				m_med.setleedTimeCODE(getCODE_R());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("leedTime", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("leedTime", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	...
	/**
	 * leedTimeクラスの標準コンストラクタ
	 *
		 */
	public leedTime(medKU0010B001 med, BatchAppConductor cdr) throws SystemException
	{
