/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/KU0010/src/com/nec/jp/orteus/metamorBase/KU0010/KU0010B001/getSEQ_ERR_WNG_LST.java,v $
 *
...
public class getSEQ_ERR_WNG_LST extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("getSEQ_ERR_WNG_LST", "read", "レコード検索条件設定処理失敗");
		...
		public boolean wasNull_CTL_SEQ_NO_R() throws FoundationException { return m_doRs.wasNull(1); }
		public Number getCTL_SEQ_NO_R() throws FoundationException { return m_doRs.getDouble(1); }
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				if(wasNull_CTL_SEQ_NO_R()==true) {
					m_med.seterrCTL_SEQ_NO(null);
				} else {
					m_med.seterrCTL_SEQ_NO(new Double(getCTL_SEQ_NO_R().doubleValue()));
				}
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("getSEQ_ERR_WNG_LST", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("getSEQ_ERR_WNG_LST", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	...
	/**
	 * getSEQ_ERR_WNG_LSTクラスの標準コンストラクタ
	 *
		 */
	public getSEQ_ERR_WNG_LST(medKU0010B001 med, BatchAppConductor cdr) throws SystemException
	{
...
+"  SEQ_ERR_WNG_LST.NEXTVAL CTL_SEQ_NO "
+"FROM "
+"  DUAL";
