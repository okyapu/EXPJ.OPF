/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/KU0010/src/com/nec/jp/orteus/metamorBase/KU0010/KU0010B001/whSu.java,v $
 *
...
public class whSu extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				setIN1_R(m_med.getwhSuIN_ITEMCD());
				setIN2_R(m_med.getwhSuIN_WHTYP());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("whSu", "read", "レコード検索条件設定処理失敗");
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				m_med.setwhSuCOUNT(getCOUNT_R());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("whSu", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("whSu", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	 ...
	 *
		 */
	public whSu(medKU0010B001 med, BatchAppConductor cdr) throws SystemException
	{
