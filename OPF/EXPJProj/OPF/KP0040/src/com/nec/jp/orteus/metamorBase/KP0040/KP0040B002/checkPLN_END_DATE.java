/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/KP0040/src/com/nec/jp/orteus/metamorBase/KP0040/KP0040B002/checkPLN_END_DATE.java,v $
 *
...
public class checkPLN_END_DATE extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				setIN5_R(m_med.getPLN_END_DATE());
				setIN6_R(m_med.getPLN_END_DATE());
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("checkPLN_END_DATE", "read", "レコード検索条件設定処理失敗");
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("checkPLN_END_DATE", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("checkPLN_END_DATE", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	...
	/**
	 * checkPLN_END_DATEクラスの標準コンストラクタ
	 *
		 */
	public checkPLN_END_DATE(medKP0040B002 med, BatchAppConductor cdr) throws SystemException
	{
...
+"  AND T_SALES_PLAN.PLN_STR_DATE <= TO_DATE(?, 'yyyyMMdd') "
+"  AND T_SALES_PLAN.PLN_END_DATE >= TO_DATE(?, 'yyyyMMdd') "
