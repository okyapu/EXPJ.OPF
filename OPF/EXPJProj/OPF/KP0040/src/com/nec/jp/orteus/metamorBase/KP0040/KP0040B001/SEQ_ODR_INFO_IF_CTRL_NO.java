/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/KP0040/src/com/nec/jp/orteus/metamorBase/KP0040/KP0040B001/SEQ_ODR_INFO_IF_CTRL_NO.java,v $
 *
...
/**
 * CLASS     : SEQ_ODR_INFO_IF_CTRL_NO クラス
 * ファイル・クラス説明
 * @author $Author: geng-jia $
 * @version $Revision: 1.13 $ $Date: 2015/12/03 08:53:21 $
 */

//{{user_implement_dev:header
//}}user_implement_dev:header

public class SEQ_ODR_INFO_IF_CTRL_NO extends AbstractBatchAppEntity
{
	...
	class interEntity
	{
		...
		public int read(boolean cache) throws BatchAppException
		{
			try {
				...
				BatchAppException be=new BatchAppException("SEQ_ODR_INFO_IF_CTRL_NO", "read", "レコード検索条件設定処理失敗");
		...
		public boolean wasNull_NEXTVAL_R() throws FoundationException { return m_doRs.wasNull(1); }
		public Number getNEXTVAL_R() throws FoundationException { return m_doRs.getDouble(1); }
		...
		public boolean next() throws BatchAppException
		{
			try {
				...
				if(wasNull_NEXTVAL_R()==true) {
					m_med.setODR_INFO_IF_CTRL_NO(null);
				} else {
					m_med.setODR_INFO_IF_CTRL_NO(new Double(getNEXTVAL_R().doubleValue()));
				}
			} catch(FoundationException e) {
				BatchAppException be=new BatchAppException("SEQ_ODR_INFO_IF_CTRL_NO", "next", "レコード移動失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		...
		public boolean hasRecord(boolean moveCursor) throws BatchAppException
		{
			...
			try {
				...
				BatchAppException be=new BatchAppException("SEQ_ODR_INFO_IF_CTRL_NO", "hasRecord", "レコード存在チェック失敗");
				be.addMessage(e.getErrmsg());
				throw be;
			}
		}
	...
	/**
	 * SEQ_ODR_INFO_IF_CTRL_NOクラスの標準コンストラクタ
	 *
		 */
	public SEQ_ODR_INFO_IF_CTRL_NO(medKP0040B001 med, BatchAppConductor cdr) throws SystemException
	{
