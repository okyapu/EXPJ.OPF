/*
 * $Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/EXPJProj/OPF/KP0040/src/com/nec/jp/orteus/metamorBase/KP0040/KP0040B001/CtrlMain.java,v $
 *
 ...
 * @version $Revision: 1.15 $ $Date: 2015/12/03 08:53:20 $
 */
		...
		implements IBatchAppCommon
{
	 ...
	 *
		 */
	public CtrlMain(medKP0040B001 med, BatchAppConductor cdr) throws BatchAppException
	{
		// メンバ変数を初期化します
		m_conductor = cdr;
		m_med = med;
	...
	*/
	 ...
	 *
	 */
	public CtrlMain(medKP0040B001 med) throws BatchAppException
	{
		// メンバ変数を初期化します
		m_med = med;
		m_control = new interControl(med);
	...
	public int main() throws BatchAppException
	{
		...
		try{
			...
			try {
				...
				String strMsgCd = "KP04101";
				String strDataString = "(SBM0583)<< 販売所要計画I/F(EDI)入力バッチ処理開始 >>";
			...
			try{
				...
				try{
					...
					for(rec = 0; fr.next(); rec++) {
						...
						if(INFO_TYP_CD == null || (!INFO_TYP_0201.equals(INFO_TYP_CD) && !INFO_TYP_0203.equals(INFO_TYP_CD))) {
							throw new Exception();
						}
						...
						if(m_med.getSEQ_ODR_INFO_IF_CTRL_NO().read() != SUCCESS){
							...
							//受注管理番号採番エラー
							this.writeLogMessage("KP04104",m_med.getSEQ_ODR_INFO_IF_CTRL_NO().toString());
							continue;
						}
						
						//=======================================
						// 売所要計画I/F(EDI)に行を追加する
						//=======================================
					...
					// コネクションを渡して、採番の準備
					logCd = new Numbering(conn,Numbering.LOG_CD,strUserId,strBusinessName,strMsgPlantCd);
					// 販売所要計画I/F(EDI)入力バッチ処理にて致命的エラーが発生しました。
					String strMsgCd = "KP04102";
			...
			//-----------------------------------------------------------------------------
				...
				try {
					...
					// 販売所要計画I/F(EDI)入力バッチ処理にて致命的エラーが発生しました。
					String strMsgCd = "KP04102";
					String strDataString = "(SBM0584)<< 販売所要計画I/F(EDI)入力バッチ異常検出 >>";
					businessMsgAcc.init(conn);
					businessMsg.setBusinessOperatingName(strBusinessName);
					businessMsg.setUser(strUserId);
					businessMsg.setPlantCode(strMsgPlantCd);
			...
			try {
				logCd = new Numbering(conn,Numbering.LOG_CD,strUserId,strBusinessName,strMsgPlantCd);
				// コネクションを渡して、業務メッセージ準備
				String strMsgCd = "KP04103";
				String strDataString = "(SBM0585)<< 販売所要計画I/F(EDI)入力バッチ処理終了 >>";
				businessMsgAcc.init(conn);
				businessMsg.setBusinessOperatingName(strBusinessName);
				businessMsg.setUser(strUserId);
				businessMsg.setPlantCode(strMsgPlantCd);
		...
		implements IBatchAppCommon
	{
		...
		public interControl(medKP0040B001 med) throws BatchAppException
		{
			// 各メンバ変数の初期化
			m_med = med;
