CREATE OR REPLACE FUNCTION FNCGETNEWTRANSLIPCD(
/*------------------------------------------------------------------------------
'
' version   : 1.00.01
'
...
'
' 備考      : 積送在庫に登録されている番号とのチェックを行い、未登録の番号を返す。
...
-- カーソルの宣言
    CURSOR curSYS_TRANSPORT_SLIP_CD_CTRL IS
    SELECT  TRANSPORT_SLIP_CD
    FROM    SYS_TRANSPORT_SLIP_CD_CTRL
    WHERE   PLANT_CD = pvc2PlantCd
    FOR UPDATE;

-- ROWTYPE の宣言
    RtpSYS_TRANSPORT_SLIP_CD_CTRL     curSYS_TRANSPORT_SLIP_CD_CTRL%ROWTYPE;

/* 定数の宣言 */
    MY_SQL_NAME             VARCHAR2(120)  := 'FncGetNewTranSlipCd';                          --ＰＬ／ＳＱＬ名
...
BEGIN
    ...
    /* 採番レコード存在確認 */
    SELECT COUNT(*) INTO intCount2 FROM SYS_TRANSPORT_SLIP_CD_CTRL
    WHERE PLANT_CD = pvc2PlantCd;
    ...
    IF intCount2 <= 0 THEN
        ...
        insert into SYS_TRANSPORT_SLIP_CD_CTRL (
                    PLANT_CD
                   ,TRANSPORT_SLIP_CD
                   ,CREATED_DATE
                   ,CREATED_BY
    ...
    OPEN curSYS_TRANSPORT_SLIP_CD_CTRL;
    FETCH curSYS_TRANSPORT_SLIP_CD_CTRL INTO RtpSYS_TRANSPORT_SLIP_CD_CTRL;
    vc2SequenceNo := RtpSYS_TRANSPORT_SLIP_CD_CTRL.TRANSPORT_SLIP_CD;
    ...
    LOOP
        ...
        /* チェック実行 */
        SELECT COUNT(*) INTO intCount FROM T_TRANSPORT_STOCK
            WHERE TRANSPORT_SLIP_CD = vc2SequenceNoPlant;

        /* 存在しないとき採番を有効 */
        IF intCount <= 0 THEN
            /*積送伝票番号採番テーブルを更新する*/
            UPDATE SYS_TRANSPORT_SLIP_CD_CTRL SET
                    TRANSPORT_SLIP_CD = vc2SequenceNo
                  , UPDATED_DATE   = SYSDATE
                  , UPDATED_BY     = pvc2UserId
                  , UPDATED_PRG_NM = pvc2BusinessName
                  , MODIFY_COUNT   = MODIFY_COUNT+1
            WHERE CURRENT OF curSYS_TRANSPORT_SLIP_CD_CTRL;
            /*処理終了*/
            EXIT;
        /* 存在するとき採番をし直す */
        ELSE
    ...
    END LOOP;
    CLOSE curSYS_TRANSPORT_SLIP_CD_CTRL;
