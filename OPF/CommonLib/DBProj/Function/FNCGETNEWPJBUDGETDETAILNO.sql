CREATE OR REPLACE FUNCTION FNCGETNEWPJBUDGETDETAILNO(
/*------------------------------------------------------------------------------
'
' version   : 1.00.01
'
...
'
' 機能説明  : 案件予算積上明細番号の採番を行う。
'           : 採番は工場毎に行い、工場コード＋連番の形式で検索結果(pvc2ResultData)に返す。
'
' 備考      : 案件予算積上明細に登録されている番号とのチェックを行い、未登録の番号を返す。
...
-- カーソルの宣言
    CURSOR curSYS_PJ_BUDGET_DTL_NO_CTRL IS
    SELECT  PJ_BUDGET_DETAIL_NO
    FROM    SYS_PJ_BUDGET_DETAIL_NO_CTRL
    WHERE   PLANT_CD = pvc2PlantCd
    FOR UPDATE;

-- ROWTYPE の宣言
    RtpSYS_PJ_BUDGET_DTL_NO_CTRL     curSYS_PJ_BUDGET_DTL_NO_CTRL%ROWTYPE;

/* 定数の宣言 */
    MY_SQL_NAME             VARCHAR2(120)  := 'FNCGETNEWPJBUDGETDETAILNO';                          --ＰＬ／ＳＱＬ名
...
BEGIN
    ...
    /* 採番レコード存在確認 */
    SELECT COUNT(*) INTO intCount2 FROM SYS_PJ_BUDGET_DETAIL_NO_CTRL
    WHERE PLANT_CD = pvc2PlantCd;
    ...
    IF intCount2 <= 0 THEN
        ...
        insert into SYS_PJ_BUDGET_DETAIL_NO_CTRL (
                    PLANT_CD
                   ,PJ_BUDGET_DETAIL_NO
                   ,CREATED_DATE
                   ,CREATED_BY
    ...
    OPEN curSYS_PJ_BUDGET_DTL_NO_CTRL;
    FETCH curSYS_PJ_BUDGET_DTL_NO_CTRL INTO RtpSYS_PJ_BUDGET_DTL_NO_CTRL;
    vc2SequenceNo := RtpSYS_PJ_BUDGET_DTL_NO_CTRL.PJ_BUDGET_DETAIL_NO;
    ...
    LOOP
        ...
        /* チェック実行 */
        SELECT COUNT(*) INTO intCount FROM T_PJ_BUDGET_DETAIL
            WHERE PJ_BUDGET_DETAIL_NO = vc2SequenceNoPlant;

        /* 存在しないとき採番を有効 */
        IF intCount <= 0 THEN
            /*案件予算積上明細番号採番テーブルを更新する*/
            UPDATE SYS_PJ_BUDGET_DETAIL_NO_CTRL SET
                    PJ_BUDGET_DETAIL_NO = vc2SequenceNo
                  , UPDATED_DATE   = SYSDATE
                  , UPDATED_BY     = pvc2UserId
                  , UPDATED_PRG_NM = pvc2BusinessName
                  , MODIFY_COUNT   = MODIFY_COUNT+1
            WHERE CURRENT OF curSYS_PJ_BUDGET_DTL_NO_CTRL;
            /*処理終了*/
            EXIT;
        /* 存在するとき採番をし直す */
        ELSE
    ...
    END LOOP;
    CLOSE curSYS_PJ_BUDGET_DTL_NO_CTRL;
