CREATE OR REPLACE FUNCTION FNCGETNEWPJODNO(
/*------------------------------------------------------------------------------
'
' version   : 1.00.01
'
...
'
' 機能説明  : 案件予算オーダデマンド番号の採番を行う。
'           : 採番は工場毎に行い、工場コード＋"PJ"＋連番の形式で検索結果(pvc2ResultData)に返す。
'
...
-- カーソルの宣言
    CURSOR curSYS_PJ_OD_NO_CTRL IS
    SELECT  PJ_OD_NO
    FROM    SYS_PJ_OD_NO_CTRL
    WHERE   PLANT_CD = pvc2PlantCd
    FOR UPDATE;

-- ROWTYPE の宣言
    RtpSYS_PJ_OD_NO_CTRL     curSYS_PJ_OD_NO_CTRL%ROWTYPE;

/* 定数の宣言 */
    MY_SQL_NAME             VARCHAR2(120)  := 'FNCGETNEWPJODNO';                          --ＰＬ／ＳＱＬ名
    ...
    SUBPRJ_ID               VARCHAR2(02)   := 'PJ';                         --案件管理ID
...
BEGIN
    ...
    /* 採番レコード存在確認 */
    SELECT COUNT(*) INTO intCount2 FROM SYS_PJ_OD_NO_CTRL
    WHERE PLANT_CD = pvc2PlantCd;
    ...
    OPEN curSYS_PJ_OD_NO_CTRL;
    FETCH curSYS_PJ_OD_NO_CTRL INTO RtpSYS_PJ_OD_NO_CTRL;
    vc2SequenceNo := RtpSYS_PJ_OD_NO_CTRL.PJ_OD_NO;
    ...
    LOOP
        ...
        /* 工場毎の番号体系へ変換 */
        vc2SequenceNoPlant := pvc2PlantCd || SUBPRJ_ID  || vc2SequenceNo;
        ...
        /* 存在しないとき採番を有効 */
        IF intCount <= 0 THEN
            /*案件予算オーダデマンド採番テーブルを更新する*/
            UPDATE SYS_PJ_OD_NO_CTRL SET
                    PJ_OD_NO = vc2SequenceNo
                  , UPDATED_DATE   = SYSDATE
                  , UPDATED_BY     = pvc2UserId
                  , UPDATED_PRG_NM = pvc2BusinessName
                  , MODIFY_COUNT   = MODIFY_COUNT+1
            WHERE CURRENT OF curSYS_PJ_OD_NO_CTRL;
            /*処理終了*/
            EXIT;
        /* 存在するとき採番をし直す */
        ELSE
