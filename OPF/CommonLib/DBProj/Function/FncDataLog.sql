CREATE OR REPLACE FUNCTION FNCDATALOG(
/*------------------------------------------------------------------------------
'
' version   : 1.00.00
'
...
------------------------------------------------------------------------------*/
   ...
   ,pvc2DataName     IN VARCHAR2            --データ名
   ,pvc2ErrMessage   IN VARCHAR2            --データ
) RETURN BOOLEAN
IS
...
BEGIN
    ...
    /* ＬＯＧモードのチェック */
    IF pvc2LogMode < LOGMODE_DATAMODE THEN
        RAISE excNORMAL_END;
    END IF;
    ...
    IF pvc2OutputMode = OUTPUTMODE_FILE THEN
        ...
        vc2TmpMessage := LOGKBN_DATA || vc2TmpDateTime || ':';
    ELSE
--        vc2TmpMessage := LOGKBN_DATA;
        vc2TmpDateTime := TO_CHAR(SYSDATE, 'YYYY/MM/DD HH24:MI:SS');
        vc2TmpMessage := LOGKBN_DATA || vc2TmpDateTime || ':';
    END IF;
