CREATE OR REPLACE FUNCTION FNCGETNEWSHIPODRNO(
/*------------------------------------------------------------------------------
'
' version   : 1.00.00
'
...
'
' 備考      : 出荷指示に登録されている番号とのチェックを行い、未登録の番号を返す。
...
-- カーソルの宣言
    CURSOR curSYS_SHIP_ODR_NO_CTRL IS
    SELECT  SHIP_ODR_NO
    FROM    SYS_SHIP_ODR_NO_CTRL
    WHERE   PLANT_CD = pvc2PlantCd
    FOR UPDATE;

-- ROWTYPE の宣言
    RtpSYS_SHIP_ODR_NO_CTRL     curSYS_SHIP_ODR_NO_CTRL%ROWTYPE;

/* 定数の宣言 */
    MY_SQL_NAME             VARCHAR2(120)  := 'FncGetNewShipOdrNo';                          --ＰＬ／ＳＱＬ名
...
BEGIN
    ...
    /* 採番レコード存在確認 */
    SELECT COUNT(*) INTO intCount2 FROM SYS_SHIP_ODR_NO_CTRL
    WHERE PLANT_CD = pvc2PlantCd;
    ...
    IF intCount2 <= 0 THEN
        ...
        insert into SYS_SHIP_ODR_NO_CTRL (
                    PLANT_CD
                   ,SHIP_ODR_NO
                   ,CREATED_DATE
                   ,CREATED_BY
    ...
    OPEN curSYS_SHIP_ODR_NO_CTRL;
    FETCH curSYS_SHIP_ODR_NO_CTRL INTO RtpSYS_SHIP_ODR_NO_CTRL;
    vc2SequenceNo := RtpSYS_SHIP_ODR_NO_CTRL.SHIP_ODR_NO;
    ...
    LOOP
        ...
        /* チェック実行 */
        SELECT COUNT(*) INTO intCount FROM T_SHIP_ODR
            WHERE SHIP_ODR_NO = vc2SequenceNoPlant;

        /* 存在しないとき採番を有効 */
        IF intCount <= 0 THEN
            /*出荷指示番号採番テーブルを更新する*/
            UPDATE SYS_SHIP_ODR_NO_CTRL SET
                    SHIP_ODR_NO = vc2SequenceNo
                  , UPDATED_DATE   = SYSDATE
                  , UPDATED_BY     = pvc2UserId
                  , UPDATED_PRG_NM = pvc2BusinessName
                  , MODIFY_COUNT   = MODIFY_COUNT+1
            WHERE CURRENT OF curSYS_SHIP_ODR_NO_CTRL;
            /*処理終了*/
            EXIT;
        /* 存在するとき採番をし直す */
        ELSE
