...
'
' 備考      : 業務メッセージに登録されている番号とのチェックを行い、未登録の番号を返す。
'           : 採番桁数は制御テーブルに登録されている採番済番号長となるが、
'           : 未登録時（初回採番）はPKGCOMPARAM.LOG_CD_LENGTHとする。
'
'
------------------------------------------------------------------------------*/
...
/* 定数の宣言 */
    MY_SQL_NAME             VARCHAR2(120)  := 'FncGetNewLogCd';                          --ＰＬ／ＳＱＬ名
    NUMBER_MAX              VARCHAR2(92)  := PKGCOMPARAM.LOG_CD_MAX;       --取得カウントアップ用
    NUMBER_MIN              VARCHAR2(92)  := PKGCOMPARAM.LOG_CD_MIN;       --取得カウントアップ用
    NUMBER_MAX_LENGTH       NUMBER        := PKGCOMPARAM.LOG_CD_LENGTH;  --採番最大桁数
