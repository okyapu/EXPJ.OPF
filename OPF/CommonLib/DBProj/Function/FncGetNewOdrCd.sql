CREATE OR REPLACE FUNCTION FNCGETNEWODRCD(
/*------------------------------------------------------------------------------
'
' version   : 1.00.02
'
...
'
' 備考      : 品目別仕掛に登録されている番号とのチェックを行い、未登録の番号を返す。
...
-- カーソルの宣言
    CURSOR curSYS_WORK_ODR_CD_CTRL IS
    SELECT  WORK_ODR_CD
    FROM    SYS_WORK_ODR_CD_CTRL
    WHERE   PLANT_CD = pvc2PlantCd
    FOR UPDATE;

-- ROWTYPE の宣言
    RtpSYS_WORK_ODR_CD_CTRL     curSYS_WORK_ODR_CD_CTRL%ROWTYPE;

/* 定数の宣言 */
    MY_SQL_NAME             VARCHAR2(120)  := 'FncGetNewOdrCd';                          --ＰＬ／ＳＱＬ名
...
BEGIN
    ...
    /* 採番レコード存在確認 */
    SELECT COUNT(*) INTO intCount2 FROM SYS_WORK_ODR_CD_CTRL
    WHERE PLANT_CD = pvc2PlantCd;
    ...
    IF intCount2 <= 0 THEN
        ...
        insert into SYS_WORK_ODR_CD_CTRL (
                    PLANT_CD
                   ,WORK_ODR_CD
                   ,CREATED_DATE
                   ,CREATED_BY
    ...
    OPEN curSYS_WORK_ODR_CD_CTRL;
    FETCH curSYS_WORK_ODR_CD_CTRL INTO RtpSYS_WORK_ODR_CD_CTRL;
    vc2SequenceNo := RtpSYS_WORK_ODR_CD_CTRL.WORK_ODR_CD;
    ...
    LOOP
        ...
        /* チェック実行 */
        SELECT COUNT(*) INTO intCount FROM T_WORK_IN_PROC_BY_ITEM
            WHERE WORK_ODR_CD = vc2SequenceNoPlant;

        /* 存在しないとき採番を有効 */
        IF intCount <= 0 THEN
            /*作業計画番号採番テーブルを更新する*/
            UPDATE SYS_WORK_ODR_CD_CTRL SET
                    WORK_ODR_CD = vc2SequenceNo
                  , UPDATED_DATE   = SYSDATE
                  , UPDATED_BY     = pvc2UserId
                  , UPDATED_PRG_NM = pvc2BusinessName
                  , MODIFY_COUNT   = MODIFY_COUNT+1
            WHERE CURRENT OF curSYS_WORK_ODR_CD_CTRL;
            /*処理終了*/
            EXIT;
        /* 存在するとき採番をし直す */
        ELSE
