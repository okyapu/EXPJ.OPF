CREATE OR REPLACE PROCEDURE SQLEXPANDBYPROC(
/*------------------------------------------------------------------------------
' Copyright (c) 2003-2004 NEC Corporation
' Copyright (c) 2003-2004 NEC Informatec Systems,Ltd.
'$Source: D:/EXPJCVS/EXPJPROJ/expjprsaUV2/DBProj/Procedure/SqlExpandByProc.sql,v $
'$Author: geng-jia $
'$Revision: 1.15 $ $Date: 2015/11/16 08:32:53 $
...
'             pvc2JobOdrCd         - (i)製番
'
' 機能説明  : オーダ発効された内作オーダに対して、作業系列展開を行い、
'             作業系列別作業計画データを作成する。
'
' 備考      :
'
...
/* カーソルの宣言 */
    ...
    --【システムパラメータ】
    CURSOR curSYS_PARAMETER IS
    SELECT  VALUE                    --作業系列メンテナンス画面のリードタイムの単位
    FROM    SYS_PARAMETER            --システムパラメータ
    WHERE   NAME = 'PROC_GRP_LT_UNIT';
    ...
    WHERE   OUTSIDE_TYP = 1  AND
            ...
            PROC_NO IN (SELECT  MIN(PROC_NO)
                        ...
                        WHERE   OUTSIDE_TYP = 1  AND
                                PLANT_CD = c_PLANT_CD AND
                                ITEM_CD = c_ITEM_CD
                        GROUP BY PLANT_CD, ITEM_CD, OUTSIDE_TYP
                       );
...
/* ROWTYPE の宣言 */
    ...
    RtpM_PROC_GRP             curM_PROC_GRP%ROWTYPE;             --【作業系列】
    RtpM_PROC_GRP_WS_CD       curM_PROC_GRP_WS_CD%ROWTYPE;       --【作業系列-作業コードチェック】
    RtpM_SBCNT_UNIT_COST      curM_SBCNT_UNIT_COST%ROWTYPE;      --【外注単価】（【取引先】，【通貨】）

/* 定数の宣言 */

    MY_SQL_NAME                         VARCHAR2(60)  := 'SQLEXPANDBYPROC';
    ...
    MRP_ODR_TYP_PHANTOM                 NUMBER(1)     := 8;   --品目手配区分(8:擬似品目)
...
/* EXCEPTION の宣言 */
    ...
    excM_PROC_GRP_Count_ERR             EXCEPTION;  --「作業系列データ件数」取得エラー
    excM_PROC_GRP_GET_ERR               EXCEPTION;  --「作業系列データ」取得エラー
    excM_PROC_GRP_WS_CD_ERR             EXCEPTION;  --「最初の内作工程に作業区コードが設定されていない場合」エラー
...
/* MESSAGE の宣言 */
    ...
    APS_NOTEXIST_M_PROC_GRP             VARCHAR2(28)     := 'AD50512';    --作業系列情報が存在しません
    ...
    APS_COM_NOTEXIST_M_CAL              VARCHAR2(28)     := 'KQ20250';	  --稼働日計算でエラーが発生しました
    APS_COM_OTHER_ERR                   VARCHAR2(28)     := 'AD00027';    --その他　Ｅｒｒｏｒ

/* LOG の宣言 */

    LOGMSG_START_PGNM_EXPANDBYPROC      VARCHAR2(104)  := '(SBM0564)<< 作業系列展開処理開始 >>';
    LOGMSG_END_PGNM_EXPANDBYPROC        VARCHAR2(104)  := '<< 作業系列展開処理終了 >>';
...
/* 変数の宣言 SQL */
    ...
    vc2PG_PROC_CD               VARCHAR2(100);   --作業系列 品目別作業コード
    numPG_PROC_NO               NUMBER(6);      --作業系列 作業系列番号
    vc2PG_PROC_NAME             VARCHAR2(160);   --作業系列 作業名
    vc2PG_WS_CD                 VARCHAR2(100);   --作業系列 作業区コード
    vc2PG_COMPANY_CD            VARCHAR2(100);   --作業系列 会社コード
    vc2PG_VEND_CD               VARCHAR2(100);   --作業系列 代表取引先コード
    numPG_FIXED_LT              NUMBER(6);      --作業系列 作業系列固定分リードタイム
    numPG_PROP_LT               NUMBER(6);      --作業系列 作業系列比例分リードタイム
    numPG_SAFETY_LT             NUMBER(6);      --作業系列 作業系列安全日数
    numPG_PROP_LOT_SIZE         NUMBER(18,4);   --作業系列 作業系列比例分ロットサイズ
    numPG_SPOIL                 NUMBER(9,4);    --作業系列 作業系列仕掛率
    numPG_OUTSIDE_TYP           NUMBER(2);      --作業系列 作業系列内外作区分
    numPG_ACPT_INSPC_TYP        NUMBER(2);      --作業系列 作業系列受入検査区分
...
/* 変数の宣言 WORK */
    ...
    numM_PROC_GRP_Count         NUMBER(10);         --「作業系列」データ数
    numM_PROC_GRP_Index         NUMBER(10);         --「作業系列」データ処理位置
    ...
    vc2PG_WS_CD_OutsideTipe     VARCHAR2(100);       --作業系列 作業区コード
    numM_PLANT_CD_Count         NUMBER(10);         --工場データ件数
    numM_PROC_WS_Count          NUMBER(10);         --作業系列-作業コードチェック件数
    ...
    numOUTSIDE_TYP_Count        NUMBER(10);         --作業系列が存在する場合の内作の数（系列が全てが外作の場合かの判断用）
    numPROC_GRP_LT_UNIT         VARCHAR2(10);       --作業系列メンテナンス画面のリードタイムの単位（1:日 2:時 3:分）
...
vc2RunChkComment := NULL;
    ...
    blnRet := FNCMSGLOG(UTL_FileHandle, pvc2LogMode, pvc2OutputMode,
                        pvc2UserId, pvc2BusinessName, pvc2PlantCd,
                        MY_SQL_NAME, APS_BATCH_START, LOGMSG_START_PGNM_EXPANDBYPROC);
    COMMIT;
...
--DBMS_OUTPUT.PUT_LINE(vc2RunChkComment);
    ...
    vc2Sql := vc2Sql || ' AND T_OD.MRP_ODR_TYP != ' || MRP_ODR_TYP_PHANTOM;
    ...
    IF pvc2WorkOdrDlvDate IS NOT NULL THEN
        ...
        IF LENGTH(pvc2WorkOdrDlvDate) <= 10 THEN
        ...
        ELSE
            vc2Sql := vc2Sql || ' AND T_WORK_IN_PROC_BY_ITEM.WORK_ODR_DLV_DATE <= TO_DATE(''' || pvc2WorkOdrDlvDate || ':59' || ''',''yyyy/mm/dd hh24:mi:ss'')';
        END IF;
    END IF;
    /* 「作業製造着手日」が指定されている場合 */
    IF pvc2OprInstStartDate IS NOT NULL THEN
        --Jf:画面からPARAMETERのFORMAT YYYY/MM/DD
        --又は、Jの時間管理=falseの場合、画面からPARAMETERのFORMAT　YYYY/MM/DD
        IF LENGTH(pvc2OprInstStartDate) <= 10 THEN
            vc2Sql := vc2Sql || ' AND T_WORK_IN_PROC_BY_ITEM.OPR_INST_START_DATE <= TO_DATE(''' || pvc2OprInstStartDate || ' 23:59:59' || ''',''yyyy/mm/dd hh24:mi:ss'')';
        --Jの時間管理=trueの場合、画面からPARAMETERのFORMAT　YYYY/MM/DDHH24:MI
        ELSE
            vc2Sql := vc2Sql || ' AND T_WORK_IN_PROC_BY_ITEM.OPR_INST_START_DATE <= TO_DATE(''' || pvc2OprInstStartDate || ':59' || ''',''yyyy/mm/dd hh24:mi:ss'')';
        END IF;
    END IF;
    /* 「製番」が指定されている場合 */
    ...
    /* 「パターンコード」 = Null */
    vc2Sql := vc2Sql || ' AND T_OD.PATTERN_CD is Null ';
/*-----------------------------------------------------------------------------
' 動的ＳＱＬの内容をトレースに出力
'
-----------------------------------------------------------------------------*/
...
--DBMS_OUTPUT.PUT_LINE(vc2RunChkComment);
            SELECT COUNT(*) INTO numM_PROC_GRP_Count
                FROM M_PROC_GRP
                WHERE ITEM_CD = vc2PBI_ITEM_CD
                    and PLANT_CD = pvc2PlantCd;
            IF numM_PROC_GRP_Count <= 0 THEN
                RAISE excM_PROC_GRP_Count_ERR;
            END IF;
/*-----------------------------------------------------------------------------
' 作業系列データ検索
'
...
--DBMS_OUTPUT.PUT_LINE(vc2RunChkComment);
            /* 「作業系列」データ処理位置初期化 */
            numM_PROC_GRP_Index := 0;
            OPEN curM_PROC_GRP(pvc2PlantCd,
                               vc2PBI_ITEM_CD);
            FETCH curM_PROC_GRP INTO RtpM_PROC_GRP;
            /* 作業系列データが存在しない場合 */
            IF curM_PROC_GRP%NOTFOUND THEN
                RAISE excM_PROC_GRP_GET_ERR;
            ELSE
/*-----------------------------------------------------------------------------
...
--DBMS_OUTPUT.PUT_LINE(vc2RunChkComment);
                    OPEN curM_PROC_GRP_WS_CD(pvc2PlantCd,
                                             vc2PBI_ITEM_CD);
                    FETCH curM_PROC_GRP_WS_CD INTO RtpM_PROC_GRP_WS_CD;
                    /* 作業区コードが設定されていない場合 */
                    IF RtpM_PROC_GRP_WS_CD.WS_CD IS null THEN
                        RAISE excM_PROC_GRP_WS_CD_ERR;
                    ELSE
                        vc2PG_WS_CD_OutsideTipe  :=  RtpM_PROC_GRP_WS_CD.WS_CD;
                    END IF;
                    CLOSE curM_PROC_GRP_WS_CD;
/*-----------------------------------------------------------------------------
...
--DBMS_OUTPUT.PUT_LINE(vc2RunChkComment);
                    ...
                    /* 「作業系列」データ処理位置 */
                    numM_PROC_GRP_Index := numM_PROC_GRP_Index + 1;
                    /* 取得データの待避 */
                    vc2PG_PROC_CD := RtpM_PROC_GRP.PROC_CD;                 --品目別作業コード
                    numPG_PROC_NO := RtpM_PROC_GRP.PROC_NO;                 --作業系列番号
                    vc2PG_PROC_NAME := RtpM_PROC_GRP.PROC_NAME;             --作業名
                    vc2PG_WS_CD := RtpM_PROC_GRP.WS_CD;                     --作業区コード
                    vc2PG_COMPANY_CD := RtpM_PROC_GRP.COMPANY_CD;           --会社コード
                    vc2PG_VEND_CD := RtpM_PROC_GRP.VEND_CD;                 --代表取引先コード
                    numPG_FIXED_LT := RtpM_PROC_GRP.FIXED_LT;               --作業系列固定分リードタイム
                    numPG_PROP_LT := RtpM_PROC_GRP.PROP_LT;                 --作業系列比例分リードタイム
                    numPG_SAFETY_LT := RtpM_PROC_GRP.SAFETY_LT;             --作業系列安全日数
                    numPG_PROP_LOT_SIZE := RtpM_PROC_GRP.PROP_LOT_SIZE;     --作業系列比例分ロットサイズ
                    numPG_SPOIL := RtpM_PROC_GRP.SPOIL;                     --作業系列仕掛率
                    numPG_OUTSIDE_TYP := RtpM_PROC_GRP.OUTSIDE_TYP;         --作業系列内外作区分
                    numPG_ACPT_INSPC_TYP := RtpM_PROC_GRP.ACPT_INSPC_TYP;   --作業系列受入検査区分
/*-----------------------------------------------------------------------------
' 作業系列毎最小許容量の計算
'
-----------------------------------------------------------------------------*/
...
--DBMS_OUTPUT.PUT_LINE(vc2RunChkComment);
                    /* 作業指示数のメモリ変数　／　作業系列.作業系列比例分ロットサイズ */
...
--DBMS_OUTPUT.PUT_LINE(vc2RunChkComment);
                    numWkGrpLT := numPG_FIXED_LT +
                                 ...
                                 numPG_SAFETY_LT;
/*-----------------------------------------------------------------------------
' 作業系列マスタのＬＴの単位を取得（1:日、2:時間、3:分）
' 
-----------------------------------------------------------------------------*/
                    OPEN curSYS_PARAMETER;
                    FETCH curSYS_PARAMETER INTO numPROC_GRP_LT_UNIT;
                    CLOSE curSYS_PARAMETER;

                    -- 1:日、2:時、3:分以外が設定されていれば1:日とみなして計算する
                    IF NVL(numPROC_GRP_LT_UNIT,'0') NOT IN ('1','2','3') THEN
                        numPROC_GRP_LT_UNIT := '1';
                    END IF;
...
-----------------------------------------------------------------------------*/
            ELSE
                    ...
                    /* 時間ベースのＬＴなら分換算する */
                    IF numPROC_GRP_LT_UNIT = '2' THEN
                        numWkGrpLT  := numWkGrpLT*60;
                    END IF;
...
--DBMS_OUTPUT.PUT_LINE(vc2RunChkComment);
                    blnStartForward := true;
                    --算出稼働日 dtmActiveDate の取得   ------FncWorkDayCalc-------
                    blnRet := FNCWORKMINUTECALC(UTL_FileHandle, pvc2LogMode, pvc2OutputMode,
                                 pvc2UserId, pvc2BusinessName, pvc2PlantCd,vc2PBI_ITEM_CD,
                                 dtmWkStartDate, numWkGrpLT, blnStartForward, dtmActiveDate);
                    IF blnRet = FALSE THEN
                        RAISE excWORK_DAY_CALC_ERR;
...
--DBMS_OUTPUT.PUT_LINE(vc2RunChkComment);
                        ...
                        IF numM_PROC_GRP_Count = numM_PROC_GRP_Index THEN
...
--DBMS_OUTPUT.PUT_LINE(vc2RunChkComment);
                            ...
                            ELSE
                                /* LTが時間または分ベースの場合、次工程の開始日時を計算する */
                                blnStartForward := true;
                                --算出稼働日 dtmActiveDate の取得   ------FncWorkDayCalc-------
                                blnRet := FNCWORKMINUTECALC(UTL_FileHandle, pvc2LogMode, pvc2OutputMode,
                                             pvc2UserId, pvc2BusinessName, pvc2PlantCd,vc2PBI_ITEM_CD,
                                             dtmActiveDate, 0, blnStartForward, dtmActiveDate);
                                IF blnRet = FALSE THEN
                                    RAISE excWORK_DAY_CALC_ERR;
                                END IF;
                                dtmWkStartDate := dtmActiveDate;
...
'
-----------------------------------------------------------------------------*/
                     IF vc2PG_VEND_CD IS NULL THEN   --作業系列.代表取引先コード
...
--DBMS_OUTPUT.PUT_LINE(vc2RunChkComment);
                    ...
                    END IF;
/*-----------------------------------------------------------------------------
' 作業系列.作業系列別実績区分＝作業系列毎に実績を上げる(1) 且つ
' 作業系列.作業系列内外作区分＝内作(1)
' 作業指示伝票を作成する
'
-----------------------------------------------------------------------------*/
...
--DBMS_OUTPUT.PUT_LINE('excITM_UNIT_QTY_TYP_GET_ERR');
                CLOSE curM_ITEM;
                /* エラーメッセージの作成 */
                vc2Comment := SUBSTR('(SBM0565)' || LOGMSG_ITEMCD || LOGMSG_BRAKETS || vc2PBI_ITEM_CD || LOGMSG_BRAKETE,
                                     1,256);
                /* エラーメッセージの出力 */
                blnRet := FNCMSGLOG(UTL_FileHandle, pvc2LogMode, pvc2OutputMode,
                                    pvc2UserId, pvc2BusinessName, pvc2PlantCd,
            ...
            --作業系列データ件数検索
            WHEN excM_PROC_GRP_Count_ERR THEN
--DBMS_OUTPUT.PUT_LINE('excM_PROC_GRP_Count_ERR');
                /* エラーメッセージの作成 */
                ...
                blnRet := FNCMSGLOG(UTL_FileHandle, pvc2LogMode, pvc2OutputMode,
                                    pvc2UserId, pvc2BusinessName, pvc2PlantCd,
                                    MY_SQL_NAME, APS_NOTEXIST_M_PROC_GRP, vc2Comment);
            --作業系列データ検索
            WHEN excM_PROC_GRP_GET_ERR THEN
--DBMS_OUTPUT.PUT_LINE('excM_PROC_GRP_GET_ERR');
                CLOSE curM_PROC_GRP;
                /* エラーメッセージの作成 */
                vc2Comment := SUBSTR('(SBM0565)' || LOGMSG_ITEMCD || LOGMSG_BRAKETS || vc2PBI_ITEM_CD || LOGMSG_BRAKETE,
                                     1,256);
                ...
                blnRet := FNCMSGLOG(UTL_FileHandle, pvc2LogMode, pvc2OutputMode,
                                    ...
                                    MY_SQL_NAME, APS_NOTEXIST_M_PROC_GRP, vc2Comment);
            --最初の内作工程に作業区コードが設定されていない場合エラー
            WHEN excM_PROC_GRP_WS_CD_ERR THEN
--DBMS_OUTPUT.PUT_LINE('excM_PROC_GRP_WS_CD_ERR');
                CLOSE curM_PROC_GRP;
                CLOSE curM_PROC_GRP_WS_CD;
                /* エラーメッセージの作成 */
                vc2Comment := SUBSTR('(SBM0496)' || LOGMSG_WORKODRCD || LOGMSG_BRAKETS || vc2PBI_WORK_ODR_CD || LOGMSG_BRAKETE,
...
--DBMS_OUTPUT.PUT_LINE('excWORK_DAY_CALC_ERR');
                ...
                blnRet := FNCMSGLOG(UTL_FileHandle, pvc2LogMode, pvc2OutputMode,
                                    pvc2UserId, pvc2BusinessName, pvc2PlantCd,
                                    MY_SQL_NAME, APS_COM_NOTEXIST_M_CAL, vc2Comment);
            --その他のエラー
            WHEN OTHERS THEN
--DBMS_OUTPUT.PUT_LINE('excOTHERS');
                ...
                vc2Comment := SUBSTR('(SBM0021)' || LOGMSG_ERR || '：' || SQLCODE || ',' || SUBSTR(SQLERRM,1,256),
                                     1,256);
                vc2Comment := vc2Comment || ' ' || LOGMSG_WORKODRCD || LOGMSG_BRAKETS || vc2PBI_WORK_ODR_CD || LOGMSG_BRAKETE;
                /* エラーメッセージの出力 */
                blnRet := FNCMSGLOG(UTL_FileHandle, pvc2LogMode, pvc2OutputMode,
                                    pvc2UserId, pvc2BusinessName, pvc2PlantCd,
...
-----------------------------------------------------------------------------*/
    ...
    /* 終了メッセージの作成 */
    vc2Comment := SUBSTR('(SBM0569)' || LOGMSG_END_PGNM_EXPANDBYPROC ||
...
--DBMS_OUTPUT.PUT_LINE('excPLANT_CDE_GET_ERR');
        ROLLBACK;
        /* 終了メッセージの作成 */
        vc2Comment := SUBSTR('(SBM0570)' || LOGMSG_END_PGNM_EXPANDBYPROC || LOGMSG_ERR ||
...
--DBMS_OUTPUT.PUT_LINE('excBUSINESSDATE_GET_ERR');
        ROLLBACK;
        /* 終了メッセージの作成 */
        vc2Comment := SUBSTR('(SBM0571)' || LOGMSG_END_PGNM_EXPANDBYPROC || LOGMSG_ERR || '：業務運用日取得',
                             1,256);
        /* 終了メッセージの出力 */
        blnRet := FNCMSGLOG(UTL_FileHandle, pvc2LogMode, pvc2OutputMode,
                            pvc2UserId, pvc2BusinessName, pvc2PlantCd,
...
--DBMS_OUTPUT.PUT_LINE('excNEW_INST_CD_GET_ERR');
        ROLLBACK;
        vc2Comment := SUBSTR('(SBM0573)' || LOGMSG_END_PGNM_EXPANDBYPROC || LOGMSG_ERR || '：作業指示番号取得',
                             1,256);
        /* 終了メッセージの出力 */
        blnRet := FNCMSGLOG(UTL_FileHandle, pvc2LogMode, pvc2OutputMode,
                            pvc2UserId, pvc2BusinessName, pvc2PlantCd,
...
--DBMS_OUTPUT.PUT_LINE('excGETROUND_ERR');
        ROLLBACK;
        vc2Comment := SUBSTR('(SBM0574)' || LOGMSG_END_PGNM_EXPANDBYPROC || LOGMSG_ERR || '：端数処理',
                             1,256);
        /* 終了メッセージの出力 */
        blnRet := FNCMSGLOG(UTL_FileHandle, pvc2LogMode, pvc2OutputMode,
                            pvc2UserId, pvc2BusinessName, pvc2PlantCd,
...
--DBMS_OUTPUT.PUT_LINE('excOTHERS');
        ROLLBACK;
        /* 終了メッセージの作成 */
        vc2Comment := SUBSTR('(SBM0575)' || LOGMSG_END_PGNM_EXPANDBYPROC || LOGMSG_ERR || '：' ||
