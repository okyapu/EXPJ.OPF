...
------------------------------------------------------------------------------*/
   ...
   ,pnumResult             OUT NUMBER      --判定結果 1: 稼働日 0: 非稼動日
...
/* 定数の宣言 */
    MY_SQL_NAME            VARCHAR2(60)  := 'SqlIsWorkDay';   --ＰＬ／ＳＱＬ名
    HOLIDAY_FLG_ACT        NUMBER(1)     := 1;                --カレンダ明細-休日フラグ(0:稼働日)
    HOLIDAY_FLG_NOTACT     NUMBER(1)     := 0;                --カレンダ明細-休日フラグ(1:非稼働日)
    ...
    STATUS_IS_WORK_DAY_ERROR     NUMBER        := 9;          -- 稼働日判定エラー
    STATUS_OTHERS_ERROR          NUMBER        := 10;         -- その他のエラー

    -- 例外の宣言
    excFncTraceLog               EXCEPTION;          --トレースログ出力エラー
    excFncIsWorkDay              EXCEPTION;          --稼働日判定エラー
