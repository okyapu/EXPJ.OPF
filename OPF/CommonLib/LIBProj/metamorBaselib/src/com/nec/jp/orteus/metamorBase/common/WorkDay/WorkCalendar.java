 ...
 */
package com.nec.jp.orteus.metamorBase.common.WorkDay;
	...
	*/
	...
	public Date calcDate() throws FoundationException{
		...
		try{
			...
			adapt.setDate(8,this.date);//対象日付
			adapt.setInt(9,this.margin);//加算日
			adapt.setInt(10,moveFlgInt);//前倒しフラグ
			adapt.setOutDate(11);//計算結果
			adapt.setOutInteger(12);//計算結果

			adapt.execUpdate();

			calcDate = adapt.getOutDate(11);
	...
	public boolean isWorkDate() throws FoundationException{
		...
		try{
			...
			adapt.setString(7,this.plantCd);//工場コード
			adapt.setDate(8,this.date);//対象日付
			adapt.setOutInteger(9);//計算結果
			adapt.setOutInteger(10);//計算結果

			adapt.execUpdate();

			Integer resultInt = adapt.getOutInteger(9);
