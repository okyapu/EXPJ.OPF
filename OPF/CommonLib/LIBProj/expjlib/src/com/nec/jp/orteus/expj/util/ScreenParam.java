...
public class ScreenParam {
	...
	*/
	 ...
	 * @throws FoundationException targetClass が null の場合
	 */
	...
	/**
	 * パッケージ名を取得します。
	 * @param	format ALL または SHORT
	 * @return	format が ALLの場合は、パッケージ名全てを返します。
	 * 			SHORTの場合は、パッケージ最下名のみを返します。
	 */
	 ...
	 * クラス名を取得します。
	 * @param	format ALL または SHORT
	 * @return	format が ALLの場合は、パッケージ名含むクラス名を返します。
	 * 			SHORTの場合は、クラス名のみを返します。
	 */
	...
	/**
	 * 画面最大表示行数を取得します。<br>
	 * 		SYS_SCREEN_PARMテーブルより、getProcId で取得した処理IDと引数で渡された key
	 * 		を条件に検索して、 MAX_DISPLAY_ROWNUM を返します。該当データが存在しない場合は
	 * 		0 を返します。
